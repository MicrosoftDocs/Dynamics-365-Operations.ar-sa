...
description: يصف هذا المقال إمكانيات عملية المخزون الداخلية لنقطة البيع (POS).
author: hhainesms
ms.date: 11/16/2022
ms.topic: article
...
يوفر الفيديو التالي نظرة عامة حول عمليات مخزون الأعمال وإمكانياته في Dynamics 365 Commerce.
...
قبل أن تتمكن مؤسستك من استخدام وظيفة العملية الداخلية، قبل أن تكوّن عملية نقطة البيع **العملية الداخلية** على تخطيط واحد أو أكثر من [‏‫تخطيطات شاشة نقطة البيع‬](/dynamics365/unified-operations/retail/pos-screen-layouts). قبل نشر العملية الجديدة في بيئة إنتاج، تأكد من اختبارها بدقه ودّرب المستخدمين لديك على استخدامها.
...
في الإصدار التجاري 10.0.14 والأحدث ، يمكن للمستخدمين استلام منتج لم يكن موجودًا في الأصل في أمر الشراء. تعمل هذه الميزة فقط لاستلام أمر الشراء. لا يمكن استلام الأصناف مقابل أوامر التحويل في حالة عدم طلب الأصناف وشحنها مسبقًا من المستودع الصادر.

لا يمكن للمستخدمين إضافة منتجات جديدة إلى أمر الشراء أثناء استلام نقطه البيع إذا كان [قد تم تمكين سير عمل أداره تغيير أمر](../supply-chain/procurement/purchase-order-approval-confirmation.md) الشراء في المركز التجاري التجاري (hq). لتمكين إدارة التغيير، يجب أولًا اعتماد كافة التغييرات التي تمت على أمر الشراء قبل السماح بالاستلام. ونظرًا لأن هذه العملية تسمح للمستلم بإضافة بنود جديدة إلى أمر الشراء، فسوف تفشل عملية الاستلام إذا تم تمكين سير عمل إدارة التغييرات. إذا تم تمكين إدارة التغيير لجميع أوامر الشراء أو للمورد المرتبط بأمر الشراء الذي يتم استلامه بشكل فعّال في نقطة الشراء، فلا يُمكن للمستخدم إضافة منتجات جديدة إلى أمر الشراء أثناء الاستلام في نقطة البيع.

لا يُمكن استخدام الوظيفة التي تتيح إضافة بنود كحل بديل لاستلام كميات إضافية من المنتجات الموجودة بالفعل في أمر الشراء. تتم إدارة الاستلام الزائد من خلال إعدادات [الاستلام الزائد](#over-receiving-validations) القياسية لبند المنتج في أمر الشراء.

عندما يتلقى المستخدم مع **عملية واردة** في نقطة البيع ، إذا قام المستخدم بمسح أو إدخال رمز شريطي للمنتج أو رقم منتج تم التعرف عليه كعنصر صالح ولكن لم يتم التعرف عليه كعنصر في أمر الشراء الحالي ، يتلقى المستخدم رسالة تطالبه بإضافة العنصر إلى الشراء ترتيب. إذا قام المستخدم بإضافة الصنف إلى أمر الشراء، فمن ثم تعتبر الكمية المُدخلة في **الاستلام الآن** الكمية المطلوبة لبند أمر الشراء.
