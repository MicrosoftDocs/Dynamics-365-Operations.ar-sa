...
هناك عناصر إضافية يجب إعدادها للقناة ويمكن العثور عليها في جزء الإجراءات ضمن قسم **الإعداد**.
...
يمكنك الاطلاع على أوضاع التسليم التي تم تكوينها عن طريق تحديد **أوضاع التسليم** من علامة تبويب **الإعداد** في جزء الإجراءات.  
...
### <a name="ensure-unique-transaction-ids"></a>التأكد من معرفات الحركات الفريدة

اعتبارًا من إصدار Commerce رقم 10.0.18، تكون معرفات الحركات التي تم إنشاؤها لنقطة البيع (POS) متسلسلة وتتضمن الأجزاء التالية:

- جزء ثابت، وهو سلسلة من معرف المتجر ومعرف الوحدة الطرفية. 
- جزء تسلسلي، وهو تسلسل رقمي. 

بشكل خاص، يكون التنسيق *{store} - {terminal} -{numbersequence}*. 

نظرا لأنه يمكن إنشاء معرفات الحركات في أوضاع متصلة وغير متصلة، فقد تم إنشاء مثيلات لمعرفات الحركات المكررة. تتطلب إزالة معرفات الحركات المكررة إصلاحًا يدويًا للبيانات. 

باستخدام إصدار Commerce رقم 10.0.19، تم تحديث تنسيق معرف الحركة لإزالة الجزء المتسلسل ويستخدم بدلاً من ذلك رقمًا مكونًا من 13 رقمًا تم إنشاؤه عن طريق حساب الوقت بالمللي ثانية منذ عام 1970. وبهذا التغيير، يكون تنسيق معرف الحركة الجديد هو *{store}-{terminal}-{millisecondsSince1970}*. يجعل هذا التحديث معرف الحركة غير متسلسل ويضمن أن تكون معرفات الحركات فريدة دائمًا. 

> [!NOTE]
> معرّفات الحركات مخصصة لاستخدام النظام الداخلي فقط، لذلك لا يلزم أن تكون متسلسلة. ومع ذلك، تتطلب العديد من البلدان أن تكون معرفات الإيصالات مسلسلة.

يمكن تمكين ميزة تنسيق معرف الحركة الجديد من مساحة عمل **إدارة الميزات**. 

لتمكين استخدام معرفات الحركات الجديدة، اتبع الخطوات التالية:

1. في Commerce headquarters، انتقل إلى **إدارة النظام \> مساحات العمل \> إدارة الميزات**.
1. قم بالتصفية لوحدة "البيع بالتجزئة والتجارة".
1. ابحث عن اسم ميزة **تمكين معرف الحركة الجديد لتجنب تكرار معرفات الحركات**.
1. حدد الميزة، ثم حدد **تمكين الآن** في الجانب الأيمن.  
1. انتقل إلى **البيع بالتجزئة والتجارة \> تكنولوجيا معلومات البيع بالتجزئة والتجارة \> جدول التوزيع**.
1. قم بتشغيل وظيفتي **تكوين قناة 1070** و **مسجل مهام نقطة بيع 1170** لمزامنة الميزة الممكّنة مع المتاجر.
1. بعد إرسال التغييرات إلى المتاجر ، يجب إغلاق وحدات نقاط البيع الطرفية وإعادة فتحها لاستخدام تنسيق معرف الحركة الجديد. 

> [!NOTE]
> بعد تمكين ميزه تنسيق معرف الحركة الجديد، لن تتمكن من تعطيل هذه الميزة. إذا كان يجب تعطيلها، فيُرجى الاتصال بدعم Commerce.
