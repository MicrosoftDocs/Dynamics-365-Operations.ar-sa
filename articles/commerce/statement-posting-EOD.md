...
> [!NOTE]
> اعتبارًا من الإصدار 10.0.14 من Commerce، عند تمكين الميزة **كشوف البيع بالتجزئة - تغذية مستمرة**، لم تعد الوظيفة الدُفعية **ترحيل المخزون** قابلة للتطبيق ويتعذر تشغيلها.
...
يسرد الجدول التالي القيم الموصى بها للمعلمات السابقة. ويجب اختبار هذه القيم وتخصيصها لتكوين النشر والبنية الأساسية المتوفرة. من شأن أي زيادة في القيم الموصي بها التأثير بشكل عكسي على معالجه الدُفعات الأخرى ويجب التحقق من صحتها.
...
| الحد الأقصى لبنود الحركة المضمنة في التجميع | ابدأ اختبار القيم عند **1000**. بحسب تكوين المركز الرئيسي، قد تكون الأوامر الصغيرة الحجم أكثر إفادةً للأداء. | تحدد هذه المعلمة عدد البنود التي سيتم تضمينها في كل أمر مبيعات أثناء ترحيل كشف الحساب. وبعد الوصول إلى هذا العدد، يتم تقسيم البنود إلى أمر جديد. علي الرغم من أن عدد بنود المبيعات لن يكون دقيقًا، نظرًا لحصول التقسيم على مستوى أمر المبيعات، فهو سيكون قريبًا من الرقم الذي تم تعيينه. تُستخدم هذه المعلمة لإنشاء أوامر المبيعات لحركات البيع بالتجزئة التي لا تحتوي على عميل مسمى. |
| الحد الأقصى لعدد سلاسل الرسائل الإلكترونية للتحقق من صحة حركات المتجر | ننصحك بتعيين هذه المعلمة إلى **4**، وبأن تقوم بزيادتها فقط إذا لم تصل إلى الأداء المطلوب. لا يمكن أن يتجاوز عدد مؤشرات الترابط التي تستخدمها هذه العملية عدد المعالجات المتوفرة لخادم الدُفعة. إذا قمت بتعيين عدد كبير من مؤشرات الترابط هنا، فقد تؤثر على معالجة دُفعة أخرى. | تتحكم هذه المعلمة في عدد الحركات التي يمكن التحقق من صحتها في نفس الوقت لمتجر معين. |
...
أثناء عملية الترحيل، يتم تجميع الحركات والمبالغ النقدية حسب العميل والمنتج. وبالتالي، يقل عدد أوامر المبيعات والبنود التي يتم إنشاؤها. ويتم تخزين الحركات المجمعة في النظام واستخدامها لإنشاء أوامر المبيعات. وتقوم كل حركة مجمعة بإنشاء أمر مبيعات مقابل في النظام. 

في حالة عدم ترحيل كشف الحساب بالكامل، فإنه يمكنك عرض الحركات المجمعة في كشف الحساب. في جزء الإجراءات، ضمن علامة التبويب **كشف حساب**، في مجموعة **تفاصيل التنفيذ**، حدد **الحركات المجمعة**.

![زر الحركات المجمعة لكشف الحساب الذي لم يتم ترحيله بالكامل.](media/aggregated-transactions.png)

بالنسبة لكشوف الحساب المرحلة، فإنه يمكنك عرض الحركات المجمعة في الصفحة **كشوف الحسابات المرحلة**. في جزء الإجراءات، حدد **استعلامات**، ثم حدد **حركات مجمعة**.

![أمر الحركات المجمعة لكشوف الحساب المرحلة.](media/aggregated-transactions-posted-statements.png)

تعرض علامة التبويب السريعة **تفاصيل أمر المبيعات** الخاصة بحركة مجمعة المعلومات التالية:
...
- **رمز الخطأ** – يتم تعيين هذا الحقل إذا كان التجميع في حالة خطأ.
- **رسالة الخطأ** – يتم تعيين هذا الحقل إذا كان التجميع في حالة خطأ. فهو يعرض تفاصيل حول ما تسبب في فشل العملية. يمكنك استخدام المعلومات الموجودة في كود الخطأ لإصلاح المشكلة، ثم إعادة تشغيل العملية يدويًا. ووفقًا لنوع الحل، قد يلزم حذف المبيعات المجمعة ومعالجتها في كشف حساب جديد.

![الحقول الموجودة في علامة التبويب السريعة "تفاصيل أمر المبيعات" لحركة مجمعة.](media/aggregated-transactions-error-message-view.png)

تعرض علامة التبويب السريعة **تفاصيل الحركة** لحركة مجمعة جميع الحركات التي تم سحبها إلى الحركة المجمعة. تُظهر البنود المجمعة في الحركة المجمعة كافة السجلات المجمعة من الحركات. وتعرض البنود المجمعة أيضا تفاصيل مثل الصنف والمتغير والكمية والسعر والمبلغ الصافي والوحدة والمستودع. بشكل أساسي، يتوافق كل بند مجمع مع بند أمر مبيعات واحد.

![علامة التبويب السريعة "تفاصيل الحركة" لحركة مجمعة.](media/aggregated-transactions-sales-details.png)

في بعض الحالات، قد تفشل الحركات المجمعة في ترحيل أمر المبيعات الموحد. في هذه الحالات، سيتم إقران رمز خطأ بحالة كشف الحساب. لعرض الحركات المجمعة التي تحتوي على أخطاء فقط، يمكنك تمكين عامل التصفية **إظهار حالات الفضل فقط** في طريقة عرض الحركات المجمعة من خلال تحديد خانة الاختيار. بتمكين عامل التصفية هذا، قم بتقييد النتائج إلى المعاملات المجمعة التي تحتوي على أخطاء تتطلب الحل. لمزيد من المعلومات حول كيفية إصلاح هذه الأخطار، راجع [تحرير وتدقيق الأوامر عبر الإنترنت ومعاملات أمر العميل غير المتزامنة](edit-order-trans.md).

![خانة اختيار لعرض عامل التصفية لعدد مرات الفشل فقط في طريقة عرض الحركات المجمعة.](media/aggregated-transactions-failure-view.png)

في الصفحة **الحركات المجمعة**، فإنه يمكنك تنزيل ملف XML لحركة مجمعة محددة عن طريق تحديد **تصدير بيانات التجميع**. يمكنك مراجعة ملف XML في أي منسق ملفات XML للاطلاع على تفاصيل البيانات الفعلية التي تتضمن إنشاء أمر المبيعات وترحيله. الأداء الوظيفي لتنزيل XML للحركات المجمعة غير متوفر لكشوف الحساب التي تم ترحيلها.

![الزر تصدير بيانات التجميع في الصفحة الحركات المجمعة.](media/aggregated-transactions-export.png)

في حالة عدم إمكانية إصلاح الخطأ عن طريق تصحيح البيانات الموجودة في أمر المبيعات أو البيانات التي تدعم أمر المبيعات، يتوفر الزر **حذف أمر العميل**. لحذف أمر، حدد الحركة المجمعة التي فشلت، ثم حدد **حذف أمر العميل**. سيتم حذف كلاً من الحركة المجمعة وأمر المبيعات المقابل. يمكنك الآن مراجعة الحركات باستخدام وظيفة التحرير والتدقيق. وبدلاً من ذلك، يمكن إعادة المعالجة من خلال كشف حساب جديد. بعد إصلاح كافة عمليات الفشل، يمكنك استئناف ترحيل كشف الحساب عن طريق تشغيل وظيفة ترحيل كشف الحساب لكشف الحساب ذا الصلة.

![الزر حذف أمر العميل في طريقة عرض الحركات المجمعة.](media/aggregated-transactions-delete-cust-order.png)

توفر طريقة عرض الحركات المجمعة الفوائد التالية:
