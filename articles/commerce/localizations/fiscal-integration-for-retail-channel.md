...
- **إعادة المحاولة** - يمكن للمشغل استخدام هذا الخيار عندما يمكن حل الفشل بسرعة ، ويمكن إعادة التسجيل المالي. على سبيل المثال، يمكن استخدام هذا الخيار عندما لا يكون الجهاز المالي متصلاً، ونفاد الورق من طابعة الضرائب المحصلة، أو هناك ازدحام ورق في طابعة الضرائب المحصلة.
- **إلغاء** - يتيح هذا الخيار للمشغل تأجيل التسجيل المالي للمعاملة أو الحدث الحالي إذا فشلت. بعد إرجاء التسجيل ، يمكن للمشغل متابعة العمل في نقطة البيع ويمكنه إكمال أي عملية غير مطلوبة من أجل التسجيل المالي. عند وقوع أي حدث يحتاج إلى تسجيل مالي في نقطة البيع (على سبيل المثال، فتح حركة جديدة)، يظهر مربع حوار معالجة الأخطاء بشكل تلقائي لإبلاغ العامل أن الحركة السابقة لم تسجل بشكل صحيح ولتوفير خيارات معالجة الأخطاء.
- **التخطي** – يمكن ان يستخدم المشغل هذا الخيار عندما لا يكون من الممكن إكمال التسجيل المالي للحركة الحالية أو الحدث الحالي ، علي سبيل المثال ، إذا كانت الطابعة المالية خارج الأمر ، ويمكن **حذف التسجيل المالي في ظل ظروف معينه**. على سبيل المثال، يمكن استخدام هذا الخيار عندما يمكن تسجيل حركة مبيعات فشلت عملية التسجيل المالي لها المالي في دفتر يومية ورقي خاص. وبعد تخطي التسجيل المالي ، يمكن متابعه العمليات العادية علي نقطه البيع. 
- **وضع علامة كمسجل** - يمكن للمشغل استخدام هذا الخيار عندما يتم تسجيل المعاملة أو الحدث الحالي بالفعل في الجهاز المالي ، على سبيل المثال تمت طباعة إيصال مالي ، ولكن يحدث فشل عندما يتم حفظ الاستجابة المالية في قاعدة بيانات القناة. بعد وضع علامة علي الحركة الحالية أو الحدث علي انه مسجل ، يمكن متابعه العمليات الدورية في نقطه البيع.
- **تأجيل‏‎**- يمكن للمشغل استخدام هذا الخيار عندما لا يتم تسجيل المعاملة لأن جهاز التسجيل أو الخدمة غير متوفرة **و** ينطبق واحد مما يلي:
    - يوجد خيار التسجيل المالي للنسخ الاحتياطي ومن الممكن متابعه عمليه التسجيل المالي للحركة الحالية. علي سبيل المثال ، يمكن ان يكون الجهاز [المالي المحلي](./latam-bra-cf-e-sat.md#scenario-4-make-a-cash-and-carry-sale-of-goods-by-using-sat-as-contingency-mode) خيار نسخ احتياطي لخدمه التسجيل المالي عبر الإنترنت في حاله عدم توفر الخدمة.
    - يمكن إكمال التسجيل المالي لاحقا بواسطة غير اطار عمل التكامل المالي. علي سبيل المثال ، يمكن فيسكالي تسجيل الحركات المؤجلة لاحقا في مجموعه من خلال وظيفة [منفصلة](./latam-bra-nfce.md#scenario-3-make-a-cash-and-carry-sale-of-goods-in-offline-contingency-mode).
    
    بعد تأجيل المعاملة أو الحدث الحالي ، يمكن متابعة العمليات المنتظمة على نقاط البيع.

> [!WARNING]
> ينبغي مراعاه خيارات **التخطي** والوضع علي السجل **والتاجيل** **واستخدامها في الحالات الاستثنائية** فقط. قم بمناقشه خيارات معالجه الأخطاء هذه باستخدام القانوني أو مرشد الضرائب وقم بتطبيق الجودجمينت الجيدة قبل تمكينها. يجب تنشيط الخيارات في عملية التسجيل المالي قبل استخدامها. للتاكد من ان عوامل التشغيل لا تستخدمها علي أساس منتظم ، يجب ان يتم منح الأذونات المقابلة إلى عوامل التشغيل.

يتم [ إنشاء الحركة](#storing-fiscal-response-in-fiscal-transaction) المالية عند تحديد الخيار **تخطي** أو **وضع علامة كمسجله** **أو تاجيل** ، ولكن لا تحتوي الحركة المالية علي استجابه مالية. ويتيح لك ذلك امكانيه التقاط حدث فشل التسجيل المالي. تسمح الخيارات تخطي ووضع علامة كمسجل وتأجيل لأكواد المعلومات بتسجيل معلومات معينة حول الفشل، مثل سبب الفشل أو تبرير تخطي التسجيل المالي أو وضع علامة على الحركة كمسجلة. للحصول على مزيد من التفاصيل حول كيفية إعداد المعلمات لمعالجة الأخطاء، راجع [تعيين إعدادات معالجة الأخطاء](setting-up-fiscal-integration-for-retail-channel.md#set-error-handling-settings).
...
إذا تم تأجيل التسجيل المالي لحركة أو حدث بعد حدوث فشل (على سبيل المثال، إذا حدد المشغل **إلغاء** في مربع حوار معالجة الأخطاء)، فيمكنك إعادة تشغيل التسجيل المالي يدويًا عن طريق استدعاء عملية مناظرة. للحصول على مزيد من التفاصيل، راجع [تفعيل التنفيذ اليدوي للتسجيل المالي المؤجل](setting-up-fiscal-integration-for-retail-channel.md#enable-manual-execution-of-deferred-fiscal-registration).
