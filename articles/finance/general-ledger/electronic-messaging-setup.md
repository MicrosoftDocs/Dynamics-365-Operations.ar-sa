...
description: توفر هذه المقالة معلومات عن كيفية إعداد وظائف الرسائل الإلكترونية (EM).
author: AdamTrukawka
ms.date: 11/18/2021
ms.topic: article
ms.prod: ''
...
- [ملء سجلات من شركات متعددة](#multiple-companies-populate)
...
| نوع الاستجابة       | حدد نوع الاستجابة لحالة الرسالة. بإمكان بعض الإجراءات في عملية المعالجة إنتاج أكثر من نوع استجابة واحد. على سبيل المثال، قد تؤدي إجراء من نوع **خدمة ويب** إلى إنتاج استجابات من النوع **‏‫تم التنفيذ بنجاح‬** أو من النوع **خطأ فني** استنادًا إلى نتيجة التنفيذ. وفي هذه الحالة، حدد حالات الرسائل لنوعي الاستجابة. لمزيد من المعلومات عن أنواع الإجراءات وأنواع الاستجابات ذات الصلة بها، راجع قسم [أنواع إجراءات معالجة الرسائل](#action-types) الوارد لاحقًا في هذه المقالة. |
...
## <a name="populate-records-from-multiple-companies"></a><a id="multiple-companies-populate"></a>ملء سجلات من شركات متعددة

إذا كان يجب أن تقوم شركتك بالإبلاغ من كيانات قانونية متعددة في قاعدة بيانات Finance نفسها، قم بإعداد [إجراءات تعبئة السجلات](#populate) لكافة الكيانات القانونية التي يجب تضمين البيانات منها في إعداد التقارير.

لتمكين هذه الإمكانية في بيئة Finance الخاصة بك، اتبع هذه الخطوات. 

1. انتقل إلى **مساحات العمل** \> **إدارة الميزات**.
2. ابحث عن الميزة **الاستعلامات بين الشركات الشقيقة لإجراءات تعبئة السجلات** وحددها في القائمة.
3. حدد **تمكين الآن**. 

لإعداد [‬‏‫إجراءات تعبئة السجلات](#populate) للعديد من الشركات التي يجب تضمين البيانات في إعداد التقارير فيها، اتبع هذه الخطوات.

1. انتقل إلى **الضريبة** \> **إعداد** \> **الرسائل الإلكترونية** \> **إجراءات تعبئة السجلات**.

    عند تمكين الميزة **الاستعلامات بين الشركات الشقيقة لإجراءات تعبئة السجلات**، تتضمن الشبكة **إعداد مصادر البيانات** الموجودة على الصفحة **‏‫إجراء تعبئة السجل‬ات** الحقل **الشركة**. بالنسبة للسجلات الموجودة التي تم إنشاؤها أثناء الإعداد العام لـ [إجراءات تعبئة السجلات](#populate)، يعرض هذا الحقل مُعرف الكيان القانوني الحالي.

2. في الشبكة **إعداد مصادر البيانات**، قم بإضافة بند لكل كيان قانوني فرعي يجب تضمينه في إعداد التقارير، وقم بتعيين الحقول التالية:

    | اسم الحقل             | قيمة |
    |------------------------|-------|
    | Name                   | أدخل قيمة النص التي ستساعدك على فهم المكان الذي يأتي منه هذا السجل. على سبيل المثال، أدخل **اسم مصدر البيانات - شركة تابعة‬ 1**. |
    | نوع صنف الرسالة      | حدد نوع صنف الرسالة المطلوب لمعالجة EM. |
    | نوع الحساب           | حدد نوع الحساب المطلوب لمعالجة EM. إذا لم يكن لمعالجة EM أنواع حسابات محددة، حدد **الكل**. |
    | اسم الجدول الرئيسي      | حدد اسم الجدول الرئيسي المطلوب لمعالجة EM. |
    | حقل رقم المستند  | حدد الحقل الذي يحتوي على رقم المستند في السجلات الخاصة بمعالجة EM. |
    | حقل تاريخ المستند    | حدد الحقل الذي يحتوي على تاريخ المستند في السجلات الخاصة بمعالجة EM. |
    | حقل حساب المستند | حدد الحقل الذي يحتوي على حساب المستند في السجلات الخاصة بمعالجة EM. |
    | الشركة                | حدد مُعرف الكيان القانوني الفرعي. |
    | استعلام المستخدم             | يتم تحديد خانة الاختيار هذه تلقائيًا عند تحديد معايير من خلال تحديد **تحرير الاستعلام**. |

3. لكل سطر جديد، حدد **تحرير الاستعلام**، وحدد معيارًا مرتبطًا بالكيان القانوني المحدد في حقل **الشركة** في السطر.
...
| الحقل                                     | الوصف |
|-------------------------------------------|-------------|
| نوع الإجراء                               | حدد نوع الإجراء. لمزيد من المعلومات عن الخيارات المتوفرة، راجع قسم [أنواع إجراء معالجة الرسائل](#action-types) الوارد لاحقًا في هذه المقالة. |
...
| اسم الملف المطلوب إرساله                         | أدخل اسم المرفق في رسالة إلكترونية يجب إرسالها بواسطة هذا الاجراء. إذا كان هناك مرفقات متعددة تحتوي على اسم الملف الأصلي نفسه، فإنه سيتم إرسال المرفق الأحدث. في حالة عدم العثور على مرفق يحتوي على اسم الملف الأصلي المحدد، فإنه سيتم إرسال الطلب بدون محتوى. يتوفر هذا الحقل فقط للإجراءات من النوع **خدمة الويب**. |
