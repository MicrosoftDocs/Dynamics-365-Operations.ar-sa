---
title: نظرة عامة على سياسات توحيد الشحنات
description: يقدم هذا المقال نظرة عامة حول الوظيفة التي توفر التكوين المرن لنُهج دمج الشحنات.
author: Mirzaab
ms.date: 05/12/2020
ms.topic: article
ms.prod: ''
...
ms.locfileid: "9427892"
---
# <a name="shipment-consolidation-policies-overview"></a>نظرة عامة على سياسات توحيد الشحنات
...
عندما *سياسات تجميع الشحنات* تم تشغيل الميزة في نظامك **تجميع الشحنة عند الإفراج عنها في المستودع** الاعدادات التي كانت متوفرة سابقًا على **المخازن** صفحة الإعداد مخفية. لمساعدتك في الانتقال إلى ميزة دمج الشحنات الجديدة، تعمل وظيفة في صفحة **نُهج دمج الشحنات** على إنشاء نهج افتراضي يتضمن الإعداد القديم تلقائيًا للمستودعات الموجودة. بعد إنشاء هذا النهج الافتراضي، لن تتم مراعات إعداد **دمج الشحنات عند الإصدار إلى المستودع** في صفحة إعداد **المستودعات** بعد ذلك. - لمزيد من المعلومات، راجع [تكوين سياسات تجميع الشحنات](configure-shipment-consolidation-policies.md).
...
| يحدد حقل الدمج التالي التوليفة الفريدة للقيم المستخدمة لدمج الشحنات *لبند تحويل*. (يتم تجاهل كافة الحقول الأخرى.)<ul><li>رقم الأمر (OrderNum)</li></ul> | تحدد حقول الدمج التالية التوليفة الفريدة للقيم المستخدمة لدمج الشحنات *لبند تحويل*. (يتم تجاهل كافة الحقول الأخرى.)<ul><li>رقم الأمر (OrderNum)</li><li>مستلم التسليم (DeliveryName)</li><li>العنوان البريدي (DeliveryPostalAddress)</li><li>كود بلد ISO (CountryRegionISOCode)</li><li>العنوان (Address)</li><li>الموقع (InventSiteId)</li><li>المستودع (InventLocationId)</li><li>شركة الشحن (CarrierCode)</li><li>خدمه الناقل (CarrierServiceCode)</li><li>وضع التسليم (ModeCode) \*</li><li>مجموعة الناقلين (CarrierGroupCode)</li><li>شروط التسليم (DlvTermId)</li></ul><p>وتعتبر هذه الحقول هي الحقول الوحيدة التي تتوفر وتتم تهيئتها عند إنشاء شحنة جديدة.</p><p>\*ملاحظة: *ModeCode* هو **الوضع** المعين إلى **شركة الشحن ‬** المحددة لبند تحويل (وليس **وضع التسليم** المحدد لبند تحويل). إذا اخترت *وضع التسليم (ModeCode)* بين معايير الدمج، فسيدمج النظام عندئذٍ بنود التحويل فقط مع قيمة **الوضع** نفسها شريطة تعريف **شركة الشحن** و **خدمة الشحن** و **وضع التسليم** كلها للبند (بصرف النظر عن قيمه). علاوةً على ذلك، سيدمج النظام أيضًا جميع بنود التحويل حيث قيمة **الوضع** فارغة.</p> |
| تحدد حقول الدمج التالية التوليفة الفريدة للقيم المستخدمة لدمج الشحنات *لبند مبيعات*. (يتم تجاهل كافة الحقول الأخرى.)<ul><li>رقم الأمر (OrderNum)</li><li>مرجع العميل (CustomerRef)</li><li>طلب العميل (CustomerReq)</li><li>شروط التسليم (DlvTermId)</li></ul> | تحدد حقول الدمج التالية التوليفة الفريدة للقيم المستخدمة لدمج الشحنات *لبند مبيعات*. (يتم تجاهل كافة الحقول الأخرى.)<ul><li>رقم الأمر (OrderNum)</li><li>رقم الحساب (AccountNum)</li><li>مستلم التسليم (DeliveryName)</li><li>العنوان البريدي (DeliveryPostalAddress)</li><li>كود بلد ISO (CountryRegionISOCode)</li><li>العنوان (Address)</li><li>الموقع (InventSiteId)</li><li>المستودع (InventLocationId)</li><li>شركة الشحن (CarrierCode)</li><li>خدمه الناقل (CarrierServiceCode)</li><li>وضع التسليم (ModeCode) \*</li><li>مجموعة الناقلين (CarrierGroupCode)</li><li>معرف الوسيط (BrokerCode)</li><li>الاتجاه (LoadDirection)</li><li>شروط التسليم (DlvTermId)</li><li>مرجع العميل (CustomerRef)</li><li>طلب العميل (CustomerReq)</li></ul><p>وتعتبر هذه الحقول هي الحقول الوحيدة التي تتوفر وتتم تهيئتها عند إنشاء شحنة جديدة.</p><p>\*ملاحظة: *ModeCode* هو **الوضع** المعين إلى **شركة الشحن ‬** المحددة لبند مبيعات (وليس **وضع التسليم** المحدد لبند مبيعات). إذا اخترت *وضع التسليم (ModeCode)* بين معايير الدمج، فسيدمج النظام عندئذٍ بنود المبيعات فقط مع قيمة **الوضع** نفسها شريطة تعريف **شركة الشحن** و **خدمة الشحن** و **وضع التسليم** كلها للبند (بصرف النظر عن قيمه). علاوةً على ذلك، سيدمج النظام أيضًا جميع بنود المبيعات حيث قيمة **الوضع** فارغة.</p> |
