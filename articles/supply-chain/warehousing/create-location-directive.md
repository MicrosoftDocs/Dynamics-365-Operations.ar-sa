...
description: يصف هذا المقال كيفيه العمل مع توجيهات الموقع. توجيهات الموقع هي القواعد المعرفة من قبل المستخدم التي تساعد في تحديد مواقع الانتقاء والتخزين لحركة المخزون.
author: Mirzaab
ms.date: 09/28/2022
ms.topic: article
ms.prod: ''
...
> [!NOTE]
> ينطبق هذا المقال على الميزات في وحدة **إدارة المستودعات**. ولا ينطبق على الميزات في وحدة [إدارة المخزون](../inventory/inventory-home-page.md).
...
1. تاكد من تشغيل مفتاح الترخيص المطلوب. انتقل إلى **إدارة النظام \> إعداد \> تكوين الترخيص**، وقم بتوسيع مفتاح الترخيص **التجاري**، ثم تحديد مفتاح التكوين **اداره النقل والمستودع**. لاحظ ان الوصول إلى المسؤول مطلوب لهذه الخطوة.
...
1. قم بإنشاء مواقع وأنواع مواقع وملفات تعريف مواقع وتنسيقات مواقع. لمزيد من المعلومات، راجع [تكوين مواقع في مستودع يدعم نظام إدارة المستودعات‬ (WMS)‬](./tasks/configure-locations-wms-enabled-warehouse.md).
1. قم بإنشاء مواقع ومناطق ومجموعات مناطق. لمزيد من المعلومات، راجع [إعداد المستودع](../../commerce/channels-setup-warehouse.md) and [تكوين المواقع في مستودع يدعم نظام إدارة المستودعات‬ (WMS)‬](./tasks/configure-locations-wms-enabled-warehouse.md).

## <a name="turn-the-location-directive-scopes-feature-on-or-off"></a><a name="scopes-feature"></a>قم بتشغيل ميزة نطاقات توجيه الموقع أو إيقاف تشغيلها

[!INCLUDE [preview-banner-section](../../includes/preview-banner-section.md)]
<!-- KFM: Preview until 10.0.31 GA -->

تمنحك ميزه نطاقات *التوجيه في الموقع* حريه أكثر عندما تقوم بتصميم توجيهات الموقع وتساعد علي تقليل التكوينات المتكررة. فهو يضيف **خيار نطاقات** ، الذي يستبدل الخيار السابق **من SKU**. في حين أن **متعددة SKU** يمكن تعيين الخيار فقط إلى *نعم* أو *لا* ال **النطاقات** لا يوفر الخيار هذين الإعدادين فقط (من خلال ملف *عنصر واحد* و *عناصر متعددة* القيم) ولكن أيضًا اثنين آخرين (من خلال *عنصر واحد أو طلب* و *كل* القيم). لمزيد من المعلومات حول هذه الإعدادات ، راجع [توجيهات المواقع FastTab](#location-directives-tab).

عند تمكينه ، فان خيار **النطاق** يستبدل خيار المضاعفة **SKU** ويكون 100-بالمائة متوافق مع التكوينات الموجودة.

لاستخدام هذه الميزة ، يجب عليك تشغيلها في نظامك. بإمكان المسؤولين استخدام إعدادات [إدارة الميزات](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) الإعدادات للتحقق من حالة الميزة وتشغيلها أو إيقاف تشغيلها. في مساحة عمل **إدارة الميزات**، تكون هذه الميزة مدرجة بالطريقة التالية:

- **الوحدة:** *إدارة المستودعات*
- **اسم الميزة:** *نطاقات التوجيه للموقع*
...
تسرد الجداول الموجودة في الأقسام الفرعية التالية الحقول الخاصة بنوع الأمر العام والخاص بالعامل-التي تتوفر عند إعداد التوجيه الخاص بالموقع.
...
| توجيهات الموقع | رمز التوجيه |
| توجيهات الموقع | نطاق *أو* وحده SKU متعددة |
| البنود | الرقم التسلسلي |
| البنود | من الكمية |
| البنود | إلى الكمية |
| السطور | الوحدة |
...
ومن هناك ، يمكنك عرض توجيهات المواقع وإنشاءها وتحريرها باستخدام الأوامر الموجودة في جزء الإجراءات. راجع الأقسام المتبقية من هذا المقال للحصول علي معلومات حول كيفيه استخدام كافة الحقول المتوفرة في الصفحة.
...
- **نسخ** – افتح مربع حوار حيث يمكنك إنشاء نسخه دقيقه للتوجيه الخاص بالموقع الحالي.
...
- **اختبارات** القبول – افتح الصفحة التي يمكنك من خلالها اعداد الاختبارات التلقائية لتحديد كيف ستعمل توجيهات الموقع الخاصة بك تحت حالات بدء مختلفه. وبهذه الطريقة ، يمكنك بسرعة التحقق من صحة التوجيهات الخاصة بك عند إنشاءها وصيانتها. لمزيد من المعلومات، راجع [اختبار توجيهات الموقع مع اختبارات القبول](location-directive-acceptance-tests.md).
...
## <a name="location-directives-fasttab"></a><a name="location-directives-tab"></a>علامة التبويب السريعة توجيهات الموقع
...
- **كود التوجيه** - حدد كود التوجيه لإقرانه بقالب العمل أو قالب التزويد. في صفحة **كود التوجيه**، يمكنك إنشاء أكواد جديدة يمكن استخدامها لتوصيل قوالب العمل أو قوالب التزويد بتوجيهات المواقع. يمكن أيضًا استخدام أكواد التوجيه لإنشاء الرابط بين أي بند في قالب عمل وتوجيه موقع (مثل باب تحميل البضائع أو موقع التجميع).
    ...
    > [!TIP]
    > إذا تم تعيين كود توجيه، لن يقوم النظام بالبحث عن توجيهات الموقع حسب الرقم التسلسلي، عند إنشاء العمل. بدلا من ذلك، سيتم البحث بواسطة الرمز الموجه. من خلال هذه الطريقة، بإمكانك أن تكون أكثر تحديدًا فيما يتعلق بتوجيه الموقع المستخدم لخطوة معينة في قالب عمل، مثل خطوة تجميع المواد.

- **النطاق** – استخدم هذا الخيار لتحديد السيناريوهات التي سيتم تطبيق التوجيه الموقع عليها. يستبدل هذا الخيار الخيار **متعدد SKU** ويتوفر فقط إذا كانت ميزه نطاقات *توجيه الموقع* قيد التشغيل في النظام لديك. (لمزيد من المعلومات، راجع [قم بتشغيل ميزة نطاقات توجيه الموقع أو إيقاف تشغيلها](#scopes-feature).)

    | اعداد النطاق | أمر مفرد بصنف واحد | أوامر متعددة بنفس الصنف | أمر مفرد يحتوي علي أصناف متعددة | أوامر متعددة مع عناصر متعددة |
    |---|---|---|---|---|
    | صنف واحد | ‏‏نعم‬ | ‏‏نعم‬ | لا | لا |
    | الأصناف المتعددة | لا | لا | ‏‏نعم‬ | ‏‏نعم‬ |
    | صنف أو أمر واحد | ‏‏نعم‬ | ‏‏نعم‬ | ‏‏نعم‬ | لا |
    | ‏‏الكل‬ | ‏‏نعم‬ | ‏‏نعم‬ | ‏‏نعم‬ | ‏‏نعم‬ |

    يوضح الجدول التالي وقت توفر النطاقات ، وما إذا كان مسموح بها لوظيفة **تحرير الاستعلام**.

    | النطاق | نوع العمل المدعوم | أنواع أوامر العمل المدعومة | السماح بتحرير الاستعلام |
    |---|---|---|---|
    | صنف واحد | ‏‏الكل‬ | ‏‏الكل‬ | ‏‏نعم‬ |
    | الأصناف المتعددة | ‏‏الكل‬ | ‏‏الكل‬ | لا |
    | صنف أو أمر واحد | يقوم | المنتج المساعد والمنتج التلو الآخر ينتهي البضائع المنتهية ، ويتم وضع كانبان علي الجانب البعيد وأوامر الشراء وأوامر التوريد والاستعاضة وأوامر الإرجاع وأوامر التوريد وإصدار التحويل وإيصال التحويل | ‏‏نعم‬ |
    | ‏‏الكل‬ | يقوم | ‏‏الكل‬ | لا |

    > [!NOTE]
    > - للقيام بوضع لكل من الأصناف المتعددة والعناصر الفردية ، يجب التاكد من وجود توجيهات المواقع التي تغطي كلا السيناريوهات. علي سبيل المثال ، قد تقوم باعداد واحد أو أكثر من *توجيهات مواقع العناصر أو الأوامر* لتغطيه السيناريوهات التي تتطلب ضبطا جيدا (مثل التحرير إلى الاستعلام) ، ثم واحد أو أكثر *من* توجيهات المواقع لتغطيه السيناريوهات المتبقية.
    > - علي الرغم من *امكانيه استخدام الصنف* المفرد ونطاقات *الأصناف* المتعددة لوضع ، فان هذا الأسلوب يؤدي عاده إلى التكوينات المتكررة. خذ بعين الاعتبار استخدام *عنصر مفرد أو ترتيب* وكافة *النطاقات* بدلا من ذلك ، لان هذا الأسلوب سينتج اعداد منظف.

- **متعددة SKU** – استخدم هذا الخيار لتحديد السيناريو الذي سيتم تطبيق توجيه الموقع عليه. يتم استبدال هذا الاعداد باعداد **النطاق** إذا كانت ميزه نطاقات *توجيه الموقع قيد التشغيل* في النظام لديك. (لمزيد من المعلومات، راجع [قم بتشغيل ميزة نطاقات توجيه الموقع أو إيقاف تشغيلها](#scopes-feature).) اضبط هذا الخيار على *نعم* لتمكين وحدات تخزين متعددة (SKU) لاستخدامها في الموقع. علي سبيل المثال، يجب تمكين وحدات SKU متعددة لموقع باب الفتحة. في حاله تمكين وحدات SKU متعددة، سيتم تحديد موقع وضعك في العمل، كما هو متوقع. ومع ذلك، سيتمكن موقع الوضع من معالجه العناصر الموجودة في الأصناف المتعددة فقط (إذا كان العمل يتضمن وحدات SKU مختلفة يجب انتقاؤها ووضعها). لن تتمكن من معالجه العبارة المفردة SKU. إذا قمت بتعيين هذا الخيار إلى *لا*، سيتم تحديد موقع الوضع فقط إذا كان للمكان الخاص بك نوعا واحدا من SKU.

    > [!IMPORTANT]
    > حتى تتمكن من اجراء كل من وضع العناصر المتعددة ووضع SKU الفردي، يجب تحديد سطرين لهما نفس البنية والإعداد، ولكن يجب تعيين خيار **وحدات SKU متعددة** إلى *نعم* لبند واحد و *لا* للآخر. لذلك بالنسبة لعمليات الوضع، سوف تحتاج إلى اثنين من توجيهات الموقع المُتطابقة، حتى لو لم تكن تميز بين وحدة SKU واحدة ووحدات SKU متعددة في مُعرف العمل. غالبا، إذا لم تقم بإعداد كل من توجيهات المواقع هذه، ستاتي مواقع العملية التجارية غير المتوقعة من توجيه الموقع المطبق. يجب استخدام إعداد مشابه لتوجيهات المواقع التي لها **نوع العمل** من *انتقاء* إذا كنت بحاجه إلى معالجه الأوامر التي تشتمل علي وحدات SKU متعددة.

    استخدم **وحدات SKU متعددة** لبنود العمل التي تعالج أكثر من رقم صنف واحد. (سيكون رقم الصنف فارغا في تفاصيل العمل، سيظهر بالشكل **متعددة** من صفحات المعالجة في تطبيق إدارة المستودع للأجهزة المحمولة.)

    في سيناريو مثال نموذجي، يتم إعداد قالب عمل بحيث يحتوي علي أكثر من زوج انتقاء/وضع واحد. في هذه الحالة، قد ترغب في البحث عن موقع مؤقت محدد لاستخدامه للبنود التي لها **نوع عمل** من *وضع*.
...
- **كود الإرجاع الساري** - حدد ما إذا كان كود الإرجاع لتوجيه الموقع يجب أن يتطابق مع كود الإرجاع الذي تم تطبيقه عند استلام الصنف، أو إذا كان يمكن تحديد توجيه الموقع بناءً على أي كود إرجاع. إذا قمت بتحديد *تطابق تام* وكان حقل **كود الإرجاع** فارغًا، فلن تراعى سوى كود الإرجاع الفارغة لتوجيه الموقع هذا.‬
    ...
    > [!NOTE]
    > يتوفر هذا الحقل فقط لأنواع أوامر العمل المحددة حيث يسمح بالتزويد. للحصول علي قائمه كامله، راجع [الحقول المحددة للمقطع أنواع العمل](#fields-specific-types) المذكور سابقا في هذا المقال.

- **حدد موقع بواسطة** - حدد ما إذا كان يجب ان تكون كميه التخزين هي الكمية الكاملة في لوحه الترخيص ام لا، أو ما إذا كان يجب ان يكون كل صنف على حدة. استخدم هذا الحقل للتاكد من وضع كافة المحتويات الموجودة في لوحه الترخيص في موقع واحد، وان النظام لا يقترح تقسيم المحتويات إلى مواقع متعددة من أجل **ASN** (استقبال لوح الترخيص)، **واستلام لوح الترخيص المختلط**، وعمليات استقبال **نظام المجموعة**. (تتطلب عمليه تلقي **نظام المجموعة** تشغيل ميزه [ميزة التخزين الخاصة بنظام المجموعة](putaway-clusters.md)). وسيتم تغيير سلوك استعلام التوجيه إلى الموقع، والبنود، وإجراءات التوجيه الخاصة بالموقع، استنادا إلى القيمة التي تحددها. لا يتم استخدام علامة التبويب السريعة **البنود** الا في حاله تعيين **تحديد حسب** إلى *الصنف*.
...
- **الإستراتيجية** - لتسهيل وتسريع عملية تحديد الإجراءات التي ترتبط بكل بند توجيه موقع، استخدم إحدى استراتيجيات المحددة مسبقاً:
    ...
    - **موقع عمل فارغ بدون عمل وارد** – استخدم هذه الاستراتيجية لتحديد المواقع الفارغة. يعتبر الموقع فارغًا إذا لم يوجد به مخزون فعلي وعند عدم وجود عمل وارد متوقع. يمكنك فقط استخدام هذه الاستراتيجية لتوجيهات الموقع لنوع العمل *وضع*.
...
بعد إنشاء توجيهات الموقع، يمكنك إقران كل كود توجيه برمز قالب عمل لإنشاء العمل. لمزيد من المعلومات، راجع [مراقبة عمل المستودع باستخدام قوالب العمل والتوجيهات المتعلقة بالموقع](./control-warehouse-location-directives.md).
...
- مقال المساعدة: [مراقبة عمل المستودع باستخدام قوالب العمل والتوجيهات المتعلقة بالموقع](control-warehouse-location-directives.md)
