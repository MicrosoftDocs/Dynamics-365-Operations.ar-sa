...
الرؤية خلف تكامل Talent مع بيئات PowerApps هي لتمكين تكامل البيانات وتدفقات الملحقات من خلال استخدام أدوات PowerApps أعلى بيانات Talent. وكنتيجة لذلك، من المهم فهم غرض بيئات PowerApps عند اختيار البيئة لاستخدامها لـ Talent. لمزيد من المعلومات حول بيئات PowerApps، بما في ذلك النطاق بيئة، والوصول إلى البيئة، وإنشاء واختيار بيئة، راجع [الإعلان عن بيئات PowerApps](https://powerapps.microsoft.com/en-us/blog/powerapps-environments/).  بينما يتم توفير كل مستأجر تلقائيًا في بيئة PowerApps افتراضية، فقد لا تكون البيئة الأفضل لاستخدامها لتوزيع Talent. يجب مراعاة إستراتيجيات تكامل بيانات والاختبار خلال هذه الخطوة، لذا نوصيك بالتفكير في الآثار المختلفة للتوزيع الخاص بك، لأنه من غير السهل التغيير فيما بعد.
...
6. تشتمل قاعدة بيانات PowerApps التي تم إنشاؤها على بيانات عرض توضيحي. بيانات العرض التوضيحي هذه مفيدة، لأنه يمكنك استخدام شركة بيانات العرض التوضيحي للاختبار أو لإنشاء تسجيلات المهام أو دلائل المهام. ومع ذلك، يتضيف بيانات العرض التوضيحي موظفين غير نشطين وعناوين وهمية، من بين معلومات أخرى، إلى بيئة الإنتاج الخاصة بك. لإزالة بيانات العرض التوضيحي، اتبع الخطوات التالية بعد الانتهاء من إنشاء قاعدة بيانات CDS:
    ...
    1. قم بتسجيل الدخول إلى [PowerApps](https://preview.web.powerapps.com/home)، وحدد البيئة التي قمت بإنشائها في الخطوة 2 من القائمة المنسدلة على الجانب الأيسر من الصفحة.
    2. قم بتوسيع **Common Data Service** في جزء التنقل الأيمن واختر **الكيانات**.
    3. على الجانب الأيمن من الصفحة، حدد زر علامة الحذف (**...**)، ثم حدد **مسح كل البيانات**.
    4. حدد **حذف البيانات** لتأكيد رغبتك في إزالة البيانات. يؤدي هذا الإجراء إلى إزالة كافة بيانات العرض التوضيحي المضمنة في CDS بشكل افتراضي. كما أنه يزيل أي بيانات أخرى تم إدخالها في قاعدة البيانات المحددة.
    
يمكنك الآن استخدام بيئتك الجديدة.

## <a name="granting-access-to-the-environment"></a>منح الوصول إلى البيئة
سيحصل المسؤول العمومي الذي قام بإنشاء البيئة على حق الوصول بشكل افتراضي، ولكن يجب منح مستخدمي التطبيقات الإضافية حق الوصول بشكل صريح. يمكن إجراء ذلك بواسطة [إضافة مستخدمين](../dev-itpro/sysadmin/tasks/create-new-users.md) و [تعيين الأدوار المناسبة لهم](../dev-itpro/sysadmin/tasks/assign-users-security-roles.md) داخل بيئة "الموارد البشرية الأساسية". بالإضافة إلى ذلك، من الضروري أيضًا إضافة هؤلاء المستخدمين إلى بيئة PowerApps حتى يمكنهم الوصول إلى تطبيقي Attract وOnboard.  قد يساعدك منشور المدونة، [تقديم مركز إدارة PowerApps](https://powerapps.microsoft.com/en-us/blog/introducing-admin-center-for-powerapps/) على إتمام تلك الخطوات المذكورة هنا:

> 1.    يجب على المسؤول العمومي الذي وزع بيئة Talent الانتقال إلى [مركز إدارة PowerApps](https://preview.admin.powerapps.com/environments).   
> 2.    حدد البيئة (البيئات) المطلوبة.
> 3.    ضمن علامة التبويب "أمان"، أضف المستخدمين الضروريين إلى دور "أداة إنشاء البيئة".

لاحظ أن هذه الخطوة الأخيرة لإضافة مستخدمين إلى بيئة PowerApps مؤقتة. سوف نقوم في النهاية بإضافة وظائف لتمكين هذا تلقائيًا عند إضافة المستخدم في "الموارد البشرية الأساسية".
