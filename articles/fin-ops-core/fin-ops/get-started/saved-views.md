...
description: توضح هذه المقالة كيفية استخدام ميزات العروض المحفوظة.
author: jasongre
ms.date: 11/21/2022
ms.topic: article
ms.prod: ''
...
- **محددات طرق العرض الكبيرة** – سيكون للصفحات التي تحتوي على قائمة بارزة محدد طريقة عرض أكبر لعدة أسباب. الأهم من ذلك، يشير محدد طريقة العرض الأكبر إلى الصفحات حيث يمكن لطريقة العرض أن تتضمن عمليات فرز وعوامل تصفية معرفة من قبل المستخدم. نظرًا لتضمين عمليات الفرز وعوامل التصفية في طرق العرض، يتم أيضًا ضمان حجم المحدد الأكبر نظرًا لأن أسماء العرض ستكون غالبًا أفضل وصف للبيانات التي تظهر على الشاشة والتوقع هو أن يقوم المستخدمون بالتبديل بين طرق العرض كثيرًا في أنواع الصفحات هذه. ويمكن أيضًا حفظ التجميع في الشبكة في طرق العرض الموجودة على صفحة مع محددات طرق عرض كبيرة. 
    
    [![محدد طريقه العرض الكبيرة الذي يدعم تعديلات الاستعلام علي طريقه العرض.](./media/views-largeViewSelector.png)](./media/views-largeViewSelector.png)
...
- **محددات طرق العرض الصغيرة** – تحتوي جميع الصفحات الأخرى في ملء الشاشة (باستثناء مساحات العمل ولوحة المعلومات) على محدد عرض أصغر يظهر بجانب التسمية التوضيحية للصفحة. تتضمن طرق العرض في هذه الصفحات التخصيصات فقط، وليس عوامل التصفية المعرفة من قبل المستخدم. في هذه الصفحات، يكون عنوان التسمية التوضيحية أو عنوان السجل المعلومات الأكثر أهمية في الجزء العلوي من الصفحة. يعكس الحجم الأصغر أيضًا لمحدد طريقة العرض التردد الأدنى لتبديل طرق العرض المتوقع في هذه الصفحات. 
    ...
    [![محدد العرض الصغير الذي لا يدعم تعديلات الاستعلام في طريقة العرض.](./media/views-smallViewSelector.png)](./media/views-smallViewSelector.png)
 
إذا حددت اسم طريقة العرض، يفتح محدد طريقة العرض ويعرض قائمة طرق العرض المتاحة للصفحة.

إذا تم تشغيل ميزة **تحسين دعم الكيان القانوني لطرق العرض المحفوظة**، يعرض محدد طريقة العرض طرق العرض المتوفرة في قسمين. يعرض القسم الأول أي طرق عرض خاصة بالكيان القانوني الحالي، ويعرض القسم الثاني طرق العرض المتوفرة لكافة الكيانات القانونية. لا يظهر القسم الأول إلا إذا كانت هناك طرق عرض خاصة بالكيان القانوني للصفحة.
...
[![تغييرات غير محفوظة في طريقة عرض.](./media/views-unsavedChanges.png)](./media/views-unsavedChanges.png)
...
3. لإنشاء طريقة عرض جديدة:
    ...
    1. حدد **حفظ باسم**. 
    2. في جزء **حفظ طريقة العرض باسم** أدخل اسما، وبشكل اختياري، وصفا لطريقة العرض.
    3. إذا كنت تريد أن تكون طريقة العرض هذه هي طريقة العرض الافتراضية، فحدد **تثبيت كإعداد افتراضي**. لمزيد من المعلومات حول طرق العرض الافتراضية راجع قسم [تغيير طريقة العرض الافتراضية](#changing-the-default-view) التالي. 
    4. إذا تم تشغيل ميزة **تحسين دعم الكيان القانوني لطرق العرض المحفوظة**، يمكنك تحديد ما إذا كنت تريد أن تكون طريقة العرض هذه متاحة لكافة الكيانات القانونية أو مجرد مجموعة فرعية منها.
    5. حدد **حفظ**.
...
> [!NOTE]
> - في ميزة **طرق العرض المحفوظة** الأساسية، توجد طريقة عرض افتراضية عمومية واحدة عبر الكيانات القانونية. إذا قمت بتغيير طريقة العرض الافتراضية، فستفتح طريقة العرض هذه بشكل افتراضي، بغض النظر عن الكيان القانوني الذي تعمل فيه حاليًا.
> - عند تشغيل ميزة **دعم الكيان القانوني المحسن لطرق العرض المحفوظة** يمكن أن يكون لكل كيان قانوني طريقة العرض الافتراضية الخاصة به لكل صفحة.
...
> [!WARNING]
> في بعض الحالات، لا يتم تنفيذ الاستعلام المرتبط بطريقة العرض الافتراضية عند عندما تفتح صفحة للمرة الأولى. على سبيل المثال، إذا فتحت الصفحة عبر تجانب، فسيتم تنفيذ استعلام التجانب بغض النظر عن الاستعلام المرتبط بطريقة العرض الافتراضية. كذلك الأمر، إذا فتحت صفحة ذات طريقة عرض **قياسية** لديها استعلام محدد، فسيتم تنفيذ الاستعلام الأصلي بدلاً من استعلام طريقة العرض الافتراضية. في هذه الحالة، ستتلقى رسالة إعلامية عندما يتم تحميل المشاهدة بإجراء مضمن يتيح لك تحميل استعلام طريقة العرض الافتراضية مباشرة. إذا قمت بالتبديل بين طرق العرض بعد تحميل الصفحة، فيجب على استعلام طريقة العرض أن يكون قادرًا على العمل كما هو متوقع. 
...
إذا تم تشغيل ميزة **تحسين دعم الكيان القانوني لطرق العرض المحفوظة** يعرض قسم **طرق العرض الخاصة بي** من مربع الحوار **إدارة طرق العرض الخاصة بي** طرق العرض المتوفرة للصفحة في الأقسام. يتم عرض أي طرق عرض خاصة بالكيان القانوني الحالي في القسم الخاص بها. يظهر قسم **طرق العرض العمومية** دائما، بحيث يمكنك إدارة طرق العرض المتوفرة للصفحة في كافة الكيانات القانونية. 

للحصول على قائمة بطرق العرض المتوفرة لهذه الصفحة، تتوفر المجموعة التالية من الإجراءات.

- **تغيير طريقة العرض الافتراضية** – استخدم الإجراء **التثبيت كافتراضي** لجعل طريقة العرض المحددة حاليًا طريقة العرض الافتراضية لهذه الصفحة. إذا تم تشغيل ميزة **استيراد دعم الكيان القانوني لطرق العرض المحفوظة** يتيح لك قسم **طرق العرض العمومية** إنشاء طريقة عرض بمثابة طريقة العرض الافتراضية للكيان القانوني الحالي أو لكافة الكيانات القانونية.
- **إعادة ترتيب طرق العرض** – استخدم الإجراءين **تحريك لأعلى** **وتحريك لأسفل** لإعادة ترتيب طرق العرض الخاصة بك في ترتيب معين.
...
ستصبح أية تغييرات يتم اجراؤها في مربع الحوار هذا نافذة المفعول بعد تحديد الزر **تحديث**.
...
6. حدد ما إذا كان يجب نشر طريقة العرض كطريقة العرض الافتراضية للمستخدمين المحددين. عند تحويل طريقة العرض إلى طريقة عرض افتراضية، سوف يراها المستخدمون في المرة التالية التي يفتحون فيها الصفحة الهدف. سيتم تغيير طريقة العرض الافتراضية العمومية الفردية لكل مستخدم مستهدف. ومع ذلك، لا يزال بإمكان المستخدمين تغيير طريقة العرض الافتراضية بعد حدوث النشر.
    ...
    > [!NOTE]
    > كن على دراية بالسلوك التالي عند نشر طريقة عرض كطريقة عرض افتراضية:
    >
    > - إذا قمت بنشر طريقة عرض كطريقة عرض افتراضية لبعض أو كل الكيانات القانونية ، يحدث السلوك التالي:
    >
    >    - إذا تم تشغيل ميزة **طرق العرض المحفوظة الأساسية** فقط، فسيتم تغيير طريقة العرض الافتراضية العمومية لكل مستخدم مستهدف. 
    >    - إذا تم تشغيل ميزة **تحسين دعم الكيان القانوني لطرق العرض المحفوظة** ونشرت طريقة العرض إلى مجموعة فرعية من الكيانات القانونية، سيتم تغيير طريقة العرض الافتراضية لتلك الكيانات القانونية لكل مستخدم مستهدف.
    >
    ...
    > - لن تعمل النشر لتعيينات الأدوار التي تم اجراؤها باستخدام مجموعات AAD. 
...
10. أضف الكيانات القانونية التي يجب ان تكون طريقة العرض هذه متاحة لها. 
    ...
    > [!NOTE]
    > كن على دراية بالسلوك التالي إذا قمت بنشر طريقة عرض لكيان قانوني معين، ولكنك لا تنشر هذا العرض كعرض افتراضي:
    >
    > - إذا تم تشغيل ميزة **طرق العرض المحفوظة الأساسية** فقط، فإن محدد طريقة عرض المستخدم للصفحة يظهر طريقة العرض أولا للكيانات القانونية المحددة فقط. ومع ذلك، بعد تحميل العرض لأول مرة، سيعرضه محدد طريقة العرض للصفحة دائمًا، بغض النظر عن الكيان القانوني.
    > - إذا تم تشغيل ميزة **تحسين دعم الكيان القانوني لطرق العرض المحفوظة** محدد طريقة العرض سيظهر فقط طريقة العرض للكيانات القانونية المحددة.
...
على الرغم من ظهور بعض قدرات الإدارة على كل صفحة، كما تمت الإشارة إليه في هذه المقالة، بإمكان **مسؤولي النظام** و **مسؤولي طرق العرض المحفوظة** إدارة طرق النظام بطريقة شاملة أكثر عبر صفحة **التخصيص**. تتضمن هذه الصفحة الأقسام والقدرات التالية على وجه التحديد: 
...
> [!NOTE]
> تتطلب ميزة **طرق العرض المحفوظة** تتطلب الميزة تمكين نظام التخصيص في تطبيقات المالية والعمليات. إذا تم إيقاف تشغيل التخصيص للبيئة بالكامل، سيتم تعطيل طرق العرض حتى لو اتبعت الخطوات المذكورة أدناه. 
...
تتوفر طرق العرض في معظم الصفحات وليس كلها. على وجه التحديد، تتوفر طرق العرض حاليًا في جميع صفحات ملء الشاشة باستثناء لوحات المعلومات.\ عرض دعم مساحات العمل متوفر من خلال دعم طرق العرض **المحفوظة لميزه مساحات العمل**. معظم الصفحات التي ليست في وضع ملء الشاشة ، والتي تتضمن مربعات حوار منسدلة وعمليات بحث ومعاينات محسّنة ، لا تدعم طرق العرض حاليًا. عرض الدعم لمربعات الحوار متاح من خلال **المحفوظة لميزه الحوارات**.
...
### <a name="why-is-a-view-that-was-published-for-a-specific-legal-entity-visible-in-all-legal-entities"></a>لماذا يتم عرض طريقة عرض تم نشرها لكيان قانوني محدد في جميع الكيانات القانونية؟

إذا قمت بنشر طريقة عرض إلى كيان قانوني معين، ولكنك لم تنشر طريقة العرض هذه كطريقة عرض افتراضية، فسيحدث السلوك التالي:

- إذا تم تشغيل ميزة **طرق العرض المحفوظة الأساسية** فقط، فإن محدد طريقة عرض المستخدم للصفحة يظهر طريقة العرض أولا للكيانات القانونية المحددة فقط. ومع ذلك، بعد تحميل العرض لأول مرة، سيعرضه محدد طريقة العرض للصفحة دائمًا، بغض النظر عن الكيان القانوني. يحدث هذا السلوك لأن المستخدمين الحصول على النسخة الشخصية الخاصة بهم من طريقة العرض المنشورة عند تحميلها، وطرق العرض الشخصية العمومية.
- إذا تم تشغيل ميزة **تحسين دعم الكيان القانوني لطرق العرض المحفوظة** محدد طريقة العرض سيظهر فقط طريقة العرض للكيانات القانونية المحددة. يحدث هذا السلوك لأن الميزة تمكن طرق العرض (بما في ذلك طرق العرض الشخصية) أن تكون مرتبطة بكيانات قانونية معينة.
