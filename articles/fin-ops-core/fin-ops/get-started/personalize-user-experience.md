...
description: توضح هذه المقالة كيف يمكنك تخصيص التطبيق.
author: jasongre
ms.date: 06/03/2022
ms.topic: article
ms.prod: ''
...
توضح هذه المقالة كيف يمكنك تخصيص التطبيق، وهو يتناول الموضوعات التالية: 
...
- استخدم الأداة **إضافة حقول** لإضافة الحقل إلى صفحتك. عند استخدام هذه الأداة، يمكنك فقط إضافة الحقول التي تعد جزءًا من تعريف الصفحة. لمزيد من المعلومات حول كيفية إنشاء الحقول الجديدة التي لا تشكل جزءًا من تعريف الصفحة الحالي، راجع [إنشاء الحقول المخصصة والتعامل معها](user-defined-fields.md). بعد تحديد الزر **إضافة حقول** على شريط الأدوات، يجب عليك أولاً تحديد الشبكة أو القسم حيث تريد إضافة حقل. سيعرض مربع حوار قائمة الحقول المرتبطة بالشبكة المحددة أو القسم المحدد. في مربع الحوار ، حدد حقلاً واحدًا أو أكثر لإضافته من قائمة **الحقول الموصى بها** أو **جميع الحقول**. بعد اختيار الحقول المطلوبة، حدد **تحديث**. لإزالة أحد الحقول التي قمت بإضافتها سابقًا، كرر العملية، ولكن قم بإلغاء تحديد الحقل في مربع الحوار.

    تعرض قائمة **الحقول الموصى بها** الحقول التي تمت إضافتها في السابق بواسطة مستخدمين آخرين في مؤسستك. يتم تحديث قائمة الحقول هذه استنادًا إلى تكرار **الوظيفة الدُفعية للتوصيات**. وهناك تجربة مماثلة عند إضافة حقول عوامل تصفية جديدة باستخدام جزء عامل التصفية على الصفحة.
...
- لإضافة تجانب إلى مساحة العمل، قم أولاً بتصفية القائمة على الصفحة بحيث تعرض البيانات التي يجب تلخيصها أو التي تريد الوصول السريع إليها. (إذا كانت ميزة **طرق العرض المحفوظة** قيد التشغيل، يتعذر عليك المتابعة حتى تقوم بحفظ طريقة عرض تتضمن هذه الشروط.) ثم حدد **إضافة إلى مساحة العمل**. حدد مساحة عمل، ثم في حقل **العرض التقديمي**، حدد **تجانب**. بعد تحديد **تكوين**، يظهر مربع حوار، حيث يمكنك تحديد التسمية التي يجب استخدامها للتجانب في مساحة العمل. يمكنك أيضًا تحديد ما إذا كان التجانب يجب أن يظهر عدد. بعد إضافة التجانب إلى مساحة العمل، يمكنك تحديدها لفتح الصفحة الحالية من مساحة العمل. ومن ثم يمكنك عرض القائمة التي تمت تصفيتها والمقترنة بالتجانب.
    - بدءًا من الإصدار 10.0.26، إذا تم تمكين **السماح للمستخدمين بتحديد أحجام التجانب وتغييرها**، يمكنك تحديد واحد من أربعة **أحجام التجانب** للتجانب الجديد الخاص بك في مربع الحوار **تكوين التجانب**. تتيح لك هذه الميزة أيضًا ضبط حجم التجانب بعد إنشائه مباشرةً من مساحة العمل.   
- لإضافة ارتباط إلى مساحة عمل، قم أولًا بتصفية القائمة على الصفحة بحيث تعرض البيانات المهمة بالنسبة لك. (إذا كانت ميزة **طرق العرض المحفوظة** قيد التشغيل، يتعذر عليك المتابعة حتى تقوم بحفظ طريقة عرض تتضمن هذه الشروط.) ثم حدد **إضافة إلى مساحة العمل**. حدد مساحة عمل، ثم في حقل **العرض التقديمي**، حدد **ارتباط**. بعد تحديد **تكوين**، يظهر مربع حوار، حيث يمكنك تحديد التسمية التي يجب استخدامها للارتباط. يمكنك أيضًا، بشكل اختياري، تحديد تسمية للقسم حيث يمكن وضع هذا الارتباط. في حالة عدم وجود هذا القسم، سيتم إنشاء قسم جديد.

> [!NOTE]
> اعتبارًا من الإصدار 10.0.25، عندما تقوم بتكوين القائمة أو الإطار المتجانب أو الارتباط، قد تحتاج أيضًا إلى تحديد طرق عرض مساحة العمل التي تريد إضافة العنصر اليها إذا تم تمكين الميزة **(إصدار أولي) دعم طرق العرض المحفوظة لمساحات العمل**. ستظهر طرق عرض مساحة العمل المتوفرة في القسم **خيارات مساحة العمل** لكل مربع حوار **تكوين**. 
...
### <a name="changing-the-size-of-a-tile"></a>تغيير حجم تجانب
بدءًا من الإصدار 10.0.26، تتيح ميزة **السماح للمستخدمين بتحديد أحجام التجانبات وتغييرها** لتعديل حجم أي تجانب غير مرتبط بمؤشر الأداء الرئيسي عن طريق التخصيص. وفي مساحة عمل، انقر بزر الماوس الأيمن فوق أحد التجانبات، ثم حدد **تخصيص**. في نافذه خاصيه التجانب، حدد الحجم المطلوب من خيارات **حجم التجانب**. سيتم ضبط حجم التجانب فورًا. إذا تم تمكين ميزة **(معاينة) طرق العرض المحفوظة لمساحات العمل**، فيمكنك حفظ هذا التخصيص في عرض مساحة العمل.  

### <a name="personalizing-your-dashboard"></a>تخصيص لوحة معلوماتك

غالبًا ما تكون لوحة المعلومات الصفحة الأولى التي ستراها عند فتح التطبيق. ويمكن تخصيصها كأي صفحة أخرى في النظام، باستخدام الآليات نفسها التي ورد وصفها في هذه المقالة. 
...
- إذا كانت ميزة **تطبيقات الصفحات الكاملة** متوقفة عن التشغيل، فيمكنك إضافة إطار متجانب بالنقر بزر الماوس الأيمن فوق عنصر على لوحة المعلومات، ثم تحديد **إضافة مساحة عمل**. يتم إنشاء تجانب مساحة عمل جديدة في أسفل لوحة المعلومات. يمكنك إعادة تسمية تجانب مساحة العمل الجديدة هذا كما تريد. يمكنك أيضًا إضافة قوائم وإطارات متجانبة وارتباطات إلى مساحة العمل على النحو الموضح في قسم [إضافة إطارات متجانبة وقوائم وارتباطات إلى مساحة العمل‬](personalize-user-experience.md#adding-tiles-lists-and-links-to-a-workspace) من هذه المقالة.
- إذا كانت ميزة **تطبيقات الصفحات الكاملة** قيد التشغيل، فيمكنك إضافة إطار متجانب بالنقر بزر الماوس الأيمن فوق عنصر على لوحة المعلومات، ثم تحديد **إضافة تطبيق**. في مربع الحوار، حدد ما إذا كنت ترغب في إضافة إطار متجانب لمساحة عمل جديده أو إطار متجانب جديد يتضمن محتوى من Power Apps أو موقع ويب. ثم اتبع الخطوات لتكوين الخيار الذي قمت بتحديده. يتم إنشاء إطار متجانب جديد أسفل لوحة المعلومات. لمزيد من المعلومات حول كيفية إضافة هذه التطبيقات المضمنة وتحريرها وحذفها ومشاركتها، راجع [تضمين تطبيقات اللوحات من Power Apps](embed-power-apps.md) و[تضمين تطبيقات الجهات الخارجية](embed-website.md).
...
بشكل عام، يمكنك تصدير واحده من طرق العرض أو عمليات التخصيص الخاصة بك عن طريق فتح الصفحة المناسبة، وفتح شريط أدوات **التخصيص**، ثم تحديد **تصدير**. لمزيد من المعلومات عن شريط الأدوات، راجع قسم [شريط أدوات التخصيص](#personalization-toolbar) السابق في هذه المقالة. وبدلاً من ذلك، في حالة تمكين [طرق العرض المحفوظة](saved-views.md)، يمكنك الانتقال إلى **الإعدادات \> خيارات المستخدم \> التخصيص** لعرض قائمة بكل تخصيصاتك في النظام. ومن هناك، يمكنك تحديد طرق العرض أو الشخصيات المطلوب تصديرها، ثم تحديد **تصدير**.
...
بالنسبة إلى العملاء الذين قاموا بتشغيل ميزة **طرق العرض المحفوظة** راجع قسم "إدارة طرق العرض بشكل عمومي" في مقالة [طرق العرض المحفوظة](saved-views.md).
...
### <a name="service-updates-and-personalization"></a>تحديثات الخدمة وإالتخصيص
لا يلزم مسح بيانات التخصيص أو إعادة تعيينها عند الانتقال إلى إصدار جديد من المنتج. سيستمر استخدام بيانات التخصيص والعروض المحفوظة في العمل عبر التحديثات. وقد تتضمن الاستثناءات التي تتم علي ذلك اجراء تغييرات علي النموذج ذاته والتي تقوم بابطال التخصيص (مثل عنصر التحكم الذي تمت ازالته) أو التغييرات في الاستعلام الأساسي للنموذج والذي يغير الشجرة الفرعية لمصدر البيانات بحيث لا يتطابق مع الشجرة الفرعية لمصدر البيانات المتصلة باستبيان طريقة العرض.
