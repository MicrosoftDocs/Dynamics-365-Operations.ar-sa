...
## <a name="configure-azure-devops"></a><span data-ttu-id="292cc-123">تكوين Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="292cc-123">Configure Azure DevOps</span></span>
...
<span data-ttu-id="292cc-173">بالنسبة لمشروع LCS، يمكنك استخدام تطبيق عميل أو مشروع شريك موجود.</span><span class="sxs-lookup"><span data-stu-id="292cc-173">For your LCS project, you can use an existing customer implementation or partner project.</span></span>
...
3. <span data-ttu-id="292cc-239">من علامة التبويب **التعليمات**، في حقل **تكوين تعليمات Lifecycle Services**، حدد مشروع LCS ذي الصلة (**RSAT** لهذا البرنامج التعليمي).</span><span class="sxs-lookup"><span data-stu-id="292cc-239">On the **Help** tab, in the **Lifecycle Services help configuration** field, select the relevant LCS project (**RSAT** for this tutorial).</span></span>
    ...
    <span data-ttu-id="292cc-241">يتم ملء مكتبات BPM ضمن مشروع LCS المناسب.</span><span class="sxs-lookup"><span data-stu-id="292cc-241">BPM libraries are filled in under the appropriate LCS project.</span></span>
...
8. <span data-ttu-id="292cc-270">إذا كنت تريد حفظ تسجيل المهمة محليًا ثم تحميلها إلى BPM عبر LCS، فاتبع الخطوات التالية:</span><span class="sxs-lookup"><span data-stu-id="292cc-270">If you want to save the task recording locally and then upload it to BPM through LCS, follow these steps:</span></span>
    ...
    3. <span data-ttu-id="292cc-275">انتقل إلى مكتبة **RSAT** BPM، وحدد عملية الأعمال لحفظ تسجيل المهمة.</span><span class="sxs-lookup"><span data-stu-id="292cc-275">Go to the **RSAT** BPM library, and select the business process to save the task recording against.</span></span>
...
### <a name="install-an-authentication-certificate"></a><span data-ttu-id="292cc-306">تثبيت شهادة مصادقة</span><span class="sxs-lookup"><span data-stu-id="292cc-306">Install an authentication certificate</span></span>
...
### <a name="install-rsat"></a><span data-ttu-id="292cc-338">تثبيت RSAT</span><span class="sxs-lookup"><span data-stu-id="292cc-338">Install RSAT</span></span>
...
### <a name="create-a-test-plan-and-test-suites"></a><span data-ttu-id="292cc-353">إنشاء خطة اختبار ومجموعة اختبار</span><span class="sxs-lookup"><span data-stu-id="292cc-353">Create a test plan and test suites</span></span>
...
5. <span data-ttu-id="292cc-438">حدد حالة الاختبار المطلوب تشغيلها، ثم حدد **تشغيل**.</span><span class="sxs-lookup"><span data-stu-id="292cc-438">Select the test case to run, and then select **Run**.</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="292cc-439">قبل تشغيل حالات الاختبار، إذا كنت تستخدم Internet Explorer كمستعرض، فتأكد من ضبط دقة سطح المكتب على **100٪** في **إعدادات عرض Windows \> تغيير الحجم والتخطيط**.</span><span class="sxs-lookup"><span data-stu-id="292cc-439">Before you run test cases, if you're using Internet Explorer as the browser, make sure that your desktop resolution is set to **100%** at **Windows Display settings \> Scale and layout**.</span></span> <span data-ttu-id="292cc-440">وإذا لم تتمكن من تغيير هذا الإعداد على جهاز ظاهري (VM)، فقم بتغييره على (الكمبيوتر المحمول) للعميل الذي تحاول الوصول إلى VM منه.</span><span class="sxs-lookup"><span data-stu-id="292cc-440">If you can't change this setting on a virtual machine (VM), change it on the client (laptop) that you're trying to access the VM from.</span></span> <span data-ttu-id="292cc-441">ويتم عندئذ توريث إعدادات الدقة من قِبل إعدادات عرض VM.</span><span class="sxs-lookup"><span data-stu-id="292cc-441">The resolution settings will then be inherited by the VM display settings.</span></span>

    ![يتم تعيين دقة سطح المكتب إلى 100%](./media/setup_rsa_tool_68.png)

6. <span data-ttu-id="292cc-443">إذا لم يتم تثبيت برامج تشغيل المستعرض في النظام، فستتلقى رسالة تحذير تنص على أن "هذه العملية تتطلب برنامج التشغيل \<اسم المستعرض\>.</span><span class="sxs-lookup"><span data-stu-id="292cc-443">If the browser drivers aren't installed in the system, you receive a warning message that states, "This operation requires the \<browser name\> driver.</span></span> <span data-ttu-id="292cc-444">هل تريد تنزيله وتثبيته تلقائيًا الآن؟ "</span><span class="sxs-lookup"><span data-stu-id="292cc-444">Do you want to automatically downloads and install it now?"</span></span> <span data-ttu-id="292cc-445">حدد **نعم**.</span><span class="sxs-lookup"><span data-stu-id="292cc-445">Select **Yes**.</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="292cc-448">إذا كنت تستخدم Chrome كمستعرض وتلقيت رسالة خطأ تنص على أنه لم يتم إنشاء الجلسة لأن إصدار Chrome غير صحيح، فقم بتنزيل أحدث برنامج تشغيل Chrome من <http://chromedriver.chromium.org/downloads> إلى المجلد **C:\\Program Files (x86)\\Regression Suite Automation Tool\\Common\\External\\Selenium**.</span><span class="sxs-lookup"><span data-stu-id="292cc-448">If you're using Chrome as the browser and receive an error message that states that the session wasn't created because the Chrome version isn't correct, download the latest Chrome driver from <http://chromedriver.chromium.org/downloads> to the **C:\\Program Files (x86)\\Regression Suite Automation Tool\\Common\\External\\Selenium** folder.</span></span>

    ![رسالة خطأ من أجل Chrome](./media/setup_rsa_tool_71.png)

    <span data-ttu-id="292cc-450">يتم تشغيل حالة الاختبار، ويتم تحديث حقل **النتيجة**.</span><span class="sxs-lookup"><span data-stu-id="292cc-450">The test case is run, and the **Result** field is updated.</span></span>

    ![مؤشر التقدم](./media/setup_rsa_tool_72.png)

    <span data-ttu-id="292cc-452">إذا اتبعت هذا البرنامج التعليمي أثناء كتابته، فستفشل حالة الاختبار **إنشاء منتج جديد**، لأن تسجيل المهام لإنشاء منتج قد حفظ اسم المنتج كقيمة برمجية مضمّنة.</span><span class="sxs-lookup"><span data-stu-id="292cc-452">If you've followed this tutorial as it's written, the **Create a new product** test case will fail, because the task recording for creating a product saved the product name as a hard-coded value.</span></span> <span data-ttu-id="292cc-453">وإذا قمت بإعادة تشغيل حالة الاختبار نفسها، يجب أن تتلقى رسالة خطأ، لأن المنتج موجود بالفعل.</span><span class="sxs-lookup"><span data-stu-id="292cc-453">If you rerun the same test case, you should receive an error message, because the product already exists.</span></span>

    ![تم تعيين حقل النتيجة إلى "فشل"](./media/setup_rsa_tool_72.png)

### <a name="view-the-test-results"></a><span data-ttu-id="292cc-455">عرض نتائج الاختبار</span><span class="sxs-lookup"><span data-stu-id="292cc-455">View the test results</span></span>

1. <span data-ttu-id="292cc-456">انقر نقرًا مزدوجًا فوق حالة الاختبار الفاشلة.</span><span class="sxs-lookup"><span data-stu-id="292cc-456">Double-click the failed test case.</span></span>

    <span data-ttu-id="292cc-457">ستتلقى رسالة خطأ</span><span class="sxs-lookup"><span data-stu-id="292cc-457">You receive an error message.</span></span>

    ![رسالة الخطأ](./media/setup_rsa_tool_73.png)

2. <span data-ttu-id="292cc-459">حدد **التفاصيل** لعرض رسالة الخطأ بالكامل.</span><span class="sxs-lookup"><span data-stu-id="292cc-459">Select **Details** to view the whole error message.</span></span>

    ![رسالة الخطأ بالكامل](./media/setup_rsa_tool_74.png)

3. <span data-ttu-id="292cc-461">لعرض نسخة مفصلة من رسالة الخطأ في Azure DevOps، حدد **فتح في Azure DevOps**.</span><span class="sxs-lookup"><span data-stu-id="292cc-461">To view a detailed version of the error message in Azure DevOps, select **Open in Azure DevOps**.</span></span> <span data-ttu-id="292cc-462">في Azure DevOps، يمكنك عرض الحالة الخاصة بحالة الاختبار ورسالة الخطأ المفصلة.</span><span class="sxs-lookup"><span data-stu-id="292cc-462">In Azure DevOps, you can view the status of the test case and the detailed error message.</span></span>

    ![رسالة خطأ مفصلة في Azure DevOps](./media/setup_rsa_tool_75.png)

4. <span data-ttu-id="292cc-464">لعرض نتائج الاختبار مباشرة في مشروع Azure DevOps، انتقل إلى **خطط الاختبار \> خطط الاختبار \> عمليات التشغيل**.</span><span class="sxs-lookup"><span data-stu-id="292cc-464">To view the test results directly in the Azure DevOps project, go to **Test Plans \> Test Plans \> Runs**.</span></span> <span data-ttu-id="292cc-465">انقر نقرًا مزدوجًا فوق تشغيل الاختبار الذي تريد رؤية المزيد من التفاصيل عنه.</span><span class="sxs-lookup"><span data-stu-id="292cc-465">Double-click the test run that you want to see more details for.</span></span>

    ![قائمة عمليات تشغيل الاختبارات في Azure DevOps](./media/setup_rsa_tool_76.png)

5. <span data-ttu-id="292cc-467">تشير علامة التبويب **تشغيل الملخص** إلى فشل حالة الاختبار، لكنها لا تقدم رسالة الخطأ الفعلية.</span><span class="sxs-lookup"><span data-stu-id="292cc-467">The **Run summary** tab indicates that the test case failed, but it doesn't provide the actual error message.</span></span> <span data-ttu-id="292cc-468">لعرض رسالة الخطأ المفصلة، حدد علامة التبويب **نتائج الاختبار**.</span><span class="sxs-lookup"><span data-stu-id="292cc-468">To view the detailed error message, select the **Test results** tab.</span></span>

    ![علامة التبويب "تشغيل الملخص"](./media/setup_rsa_tool_77.png)

    <span data-ttu-id="292cc-470">توفر علامة التبويب **نتائج الاختبار** معلومات حالة الاختبار، إلى جانب النتيجة ورسالة الخطأ.</span><span class="sxs-lookup"><span data-stu-id="292cc-470">The **Test results** tab provides the test case information, together with the outcome and the error message.</span></span>

    ![علامة التبويب "نتائج الاختبار"](./media/setup_rsa_tool_78.png)

6. <span data-ttu-id="292cc-472">انقر نقرًا مزدوجًا فوق السجل المعني لعرض رسالة الخطأ المفصلة.</span><span class="sxs-lookup"><span data-stu-id="292cc-472">Double-click the relevant record to view the detailed error message.</span></span>

    ![رسالة خطأ مفصلة](./media/setup_rsa_tool_79.png)

    > [!NOTE]
    > <span data-ttu-id="292cc-474">تتوفر أيضًا جميع رسائل الخطأ محليًا في **C:\\Users\\\$YourUserName\\AppData\\Roaming\\regressionTool\\errormsg-.txt**.</span><span class="sxs-lookup"><span data-stu-id="292cc-474">All error messages are also available locally in **C:\\Users\\\$YourUserName\\AppData\\Roaming\\regressionTool\\errormsg-.txt**.</span></span>

7. <span data-ttu-id="292cc-475">يمكنك أيضًا تصدير نتائج تشغيل الاختبار من مستوى خطة الاختبار عن طريق تحديد **تصدير**.</span><span class="sxs-lookup"><span data-stu-id="292cc-475">You can also export the test run results from the test plan level by selecting **Export**.</span></span>

    ![تصدير خطة اختبار](./media/setup_rsa_tool_80.png)

### <a name="modify-the-excel-parameter-file"></a><span data-ttu-id="292cc-477">تعديل ملف معلمة Excel</span><span class="sxs-lookup"><span data-stu-id="292cc-477">Modify the Excel parameter file</span></span>

1. <span data-ttu-id="292cc-478">افتح RSAT..</span><span class="sxs-lookup"><span data-stu-id="292cc-478">Open RSAT.</span></span>
2. <span data-ttu-id="292cc-479">حدد حالات الاختبار، ثم حدد **تحرير** لفتح ملفات معلمة Excel.</span><span class="sxs-lookup"><span data-stu-id="292cc-479">Select the test case, and then select **Edit** to open the Excel parameter file.</span></span>

    <span data-ttu-id="292cc-480">في الورقة **EcoResProductCreate**، لاحظ أن قيمة حقل **رقم المنتج** هي قيمة برمجية مضمّنة.</span><span class="sxs-lookup"><span data-stu-id="292cc-480">On the **EcoResProductCreate** sheet, notice that the value of the **Product number** field is hard-coded.</span></span> <span data-ttu-id="292cc-481">ويجب تحديث هذا الحقل إلى رقم منتج جديد قبل تشغيل حالة الاختبار مرة أخرى.</span><span class="sxs-lookup"><span data-stu-id="292cc-481">You must update this field to a new product number before you run the test case again.</span></span>

3. <span data-ttu-id="292cc-482">لإنشاء رقم منتج فريد لكل عملية تشغيل دون الحاجة إلى إعادة فتح ملف معلمة Excel وتحديث رقم المنتج يدويًا في كل مرة، استخدم معادلة Excel التالية.</span><span class="sxs-lookup"><span data-stu-id="292cc-482">To generate a unique product number for each run without having to reopen the Excel parameter file and manually update the product number every time, use the following Excel formula.</span></span>
    ...
    > [!NOTE]
    > <span data-ttu-id="292cc-483">بالإضافة إلى علامة التبويب **عام**، يحتوي ملف معلمة Excel على علامة تبويب بيانات لكل صفحة نموذج تقوم حالة الاختبار بزيارتها.</span><span class="sxs-lookup"><span data-stu-id="292cc-483">In addition to the **General** tab, the Excel parameter file contains a data tab for every form page the test case visits.</span></span>

    ![حقل رقم المنتج](./media/setup_rsa_tool_81.png)

4. <span data-ttu-id="292cc-485">حدد **حفظ**، ثم قم بإغلاق مصنف Excel.</span><span class="sxs-lookup"><span data-stu-id="292cc-485">Select **Save**, and then close the Excel workbook.</span></span>
5. <span data-ttu-id="292cc-486">حدد **تحميل** لحفظ ملف معلمة Excel إلى Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="292cc-486">Select **Upload** to save the Excel parameter file to Azure DevOps.</span></span>

    ![رسالة التحميل بنجاح](./media/setup_rsa_tool_82.png)

    > [!NOTE]
    > <span data-ttu-id="292cc-488">لتشغيل حالات الاختبار في سياق مستخدم معين، أدخل معرف البريد الإلكتروني للمستخدم في حقل **مستخدم الاختبار** في علامة التبويب **عام** لملف معلمة Excel.</span><span class="sxs-lookup"><span data-stu-id="292cc-488">To run test cases in a specific user context, enter the user's email ID in the **Test User** field on the **General** tab of the Excel parameter file.</span></span> <span data-ttu-id="292cc-489">وفي أحدث إصدار من RSAT، تم تحديث تخطيط الحقول في ملف معلمة Excel، لكن ظل المفهوم كما هو.</span><span class="sxs-lookup"><span data-stu-id="292cc-489">In the latest version of RSAT, the layout of the fields in the Excel parameter file has been updated, but the concept remains the same.</span></span>
    >
    > ![حقل مستخدم الاختبار](./media/setup_rsa_tool_83.png)

### <a name="validate-the-results"></a><span data-ttu-id="292cc-491">التحقق من صحة النتائج</span><span class="sxs-lookup"><span data-stu-id="292cc-491">Validate the results</span></span>

- <span data-ttu-id="292cc-492">حدد **تشغيل** لإعادة تشغيل حالة الاختبار، وتحقق من ‏‫اجتياز‬ حالة الاختبار.</span><span class="sxs-lookup"><span data-stu-id="292cc-492">Select **Run** to rerun the test case, and verify that the test case has passed.</span></span> <span data-ttu-id="292cc-493">ويمكنك عرض نتائج الاختبار كما هو موضح في قسم [عرض نتائج الاختبار](#view-the-test-results).</span><span class="sxs-lookup"><span data-stu-id="292cc-493">You can view the test results as described in the [View the test results](#view-the-test-results) section.</span></span>

    ![تم تعيين حقل النتيجة إلى "‏‫تم الاجتياز‬"](./media/setup_rsa_tool_84.png)

### <a name="chaining-of-test-cases"></a><span data-ttu-id="292cc-495">تسلسل حالات الاختبار</span><span class="sxs-lookup"><span data-stu-id="292cc-495">Chaining of test cases</span></span>

<span data-ttu-id="292cc-496">تتمثل إحدى الميزات الرئيسية لـ RSAT في تسلسل حالات الاختبار (أي، قدرة الاختبار على اجتياز القيم إلى اختبارات أخرى).</span><span class="sxs-lookup"><span data-stu-id="292cc-496">One key feature of RSAT is the chaining of test cases (that is, the capability of a test to pass values to other tests).</span></span> <span data-ttu-id="292cc-497">ويتم تشغيل حالات الاختبار وفقًا لترتيبها المحدد في خطة اختبار Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="292cc-497">Test cases are run according to their defined order in the Azure DevOps test plan.</span></span> <span data-ttu-id="292cc-498">(يمكن أيضًا تحديث هذا الطلب في أداة الاختبار نفسها.) لذا إذا كنت تريد اجتياز المتغيرات من حالة اختبار إلى أخرى، فمن المهم جدًا أن تكون الاختبارات بالترتيب الصحيح.</span><span class="sxs-lookup"><span data-stu-id="292cc-498">(This order can also be updated in the test tool itself.) Therefore, if you want to pass variables from one test case to another, it's very important that the tests be in the correct order.</span></span>

<span data-ttu-id="292cc-499">في هذا القسم، ستقوم بإنشاء متغير محفوظ في حالة الاختبار الأول وإنشاء حالة اختبار ثانية واجتياز المتغير المحفوظ من حالة الاختبار الأولى إلى حالة الاختبار الثانية.</span><span class="sxs-lookup"><span data-stu-id="292cc-499">In this section, you will create a saved variable in the first test case, create a second test case, and pass the saved variable from the first test case to the second test case.</span></span> <span data-ttu-id="292cc-500">وستقوم بعد ذلك بتشغيل حالات الاختبار كحالة اختبار متسلسلة في RSAT.</span><span class="sxs-lookup"><span data-stu-id="292cc-500">You will then run the test cases as a chained test case in RSAT.</span></span>

#### <a name="modify-an-existing-task-recording-to-create-a-saved-variable"></a><span data-ttu-id="292cc-501">تعديل تسجيل مهام موجود لإنشاء متغير محفوظ</span><span class="sxs-lookup"><span data-stu-id="292cc-501">Modify an existing task recording to create a saved variable</span></span>

1. <span data-ttu-id="292cc-502">افتح العميل.</span><span class="sxs-lookup"><span data-stu-id="292cc-502">Open the client.</span></span>
2. <span data-ttu-id="292cc-503">حدد زر **الإعدادات** (رمز الترس)، ثم حدد **مسجل المهام**.</span><span class="sxs-lookup"><span data-stu-id="292cc-503">Select the **Settings** button (the gear symbol), and then select **Task recorder**.</span></span>
3. <span data-ttu-id="292cc-504">حدد **تحرير التسجيل**.</span><span class="sxs-lookup"><span data-stu-id="292cc-504">Select **Edit Recording**.</span></span>

    ![زر تحرير التسجيل](./media/setup_rsa_tool_85.png)

4. <span data-ttu-id="292cc-506">حدد **فتح من Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="292cc-506">Select **Open from Lifecycle Services**.</span></span>

    ![زر الفتح من Lifecycle Services](./media/setup_rsa_tool_86.png)

5. <span data-ttu-id="292cc-508">حدد **تحديد مكتبة Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="292cc-508">Select **Select the Lifecycle Services library**.</span></span>

    ![زر تحديد مكتبة Lifecycle Services](./media/setup_rsa_tool_87.png)

    <span data-ttu-id="292cc-510">يتم تحميل مكتبات BPM من LCS.</span><span class="sxs-lookup"><span data-stu-id="292cc-510">BPM libraries are loaded from LCS.</span></span>

    ![مؤشر التقدم](./media/setup_rsa_tool_88.png)

6. <span data-ttu-id="292cc-512">بعد تحميل مكتبات BPM من LCS، حدد مكتبة **RSAT** BPM وعملية الأعمال **إنشاء منتج أعمال جديد** التي كان تسجيل المهام مقترنًا بها .</span><span class="sxs-lookup"><span data-stu-id="292cc-512">After the BPM libraries are loaded from LCS, select the **RSAT** BPM library and the **Create a new product** business process that the task recording was associated with.</span></span> <span data-ttu-id="292cc-513">ثم حدد **موافق**.</span><span class="sxs-lookup"><span data-stu-id="292cc-513">Then select **OK**.</span></span>

    ![تحديد مكتبة BPM وعملية أعمال](./media/setup_rsa_tool_89.png)

7. <span data-ttu-id="292cc-515">يتم إدخال اسم تسجيل المهام المناسب في حقل **اسم التسجيل**.</span><span class="sxs-lookup"><span data-stu-id="292cc-515">The name of the appropriate task recording is entered in the **Recording name** field.</span></span> <span data-ttu-id="292cc-516">حدد **بدء**.</span><span class="sxs-lookup"><span data-stu-id="292cc-516">Select **Start**.</span></span>

    ![اسم تسجيل المهام في الحقل "اسم التسجيل"](./media/setup_rsa_tool_90.png)

8. <span data-ttu-id="292cc-518">انتقل إلى **إدارة معلومات المنتجات \> المنتجات**، وحدد **جديد** لفتح الصفحة التي تم فيها تسجيل المهمة الأصلية، **إنشاء منتج**.</span><span class="sxs-lookup"><span data-stu-id="292cc-518">Go to **Product information management \> Products**, and select **New** to open the page where the original task recording, **Create a product**, was recorded.</span></span>
9. <span data-ttu-id="292cc-519">حدد **إدراج خطوه**.</span><span class="sxs-lookup"><span data-stu-id="292cc-519">Select **Insert step**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="292cc-520">يتم إدراج الخطوة الجديدة **بعد** الخطوة التي حددتها في الجزء.</span><span class="sxs-lookup"><span data-stu-id="292cc-520">The new step is inserted **after** the step that you selected in the pane.</span></span>

    ![زر إدراج خطوة](./media/setup_rsa_tool_91.png)

10. <span data-ttu-id="292cc-522">انقر بزر الماوس الأيمن فوق الحقل **رقم المنتج**، ثم حدد **مسجل المهام \> نسخ**.</span><span class="sxs-lookup"><span data-stu-id="292cc-522">Right-click the **Product number** field, and then select **Task recorder \> Copy**.</span></span>

    ![أمر النسخ](./media/setup_rsa_tool_92.png)

11. <span data-ttu-id="292cc-524">تمت إضافة خطوة جديدة في الجزء.</span><span class="sxs-lookup"><span data-stu-id="292cc-524">A new step is added in the pane.</span></span> <span data-ttu-id="292cc-525">قم بتدوين القيمة في الحقل **رقم المنتج**، لأنك ستحتاج إليها لاحقًا.</span><span class="sxs-lookup"><span data-stu-id="292cc-525">Make a note of the value in the **Product number** field, because you will need it later.</span></span>

    ![تمت إضافة خطوة جديدة](./media/setup_rsa_tool_93.png)

12. <span data-ttu-id="292cc-527">حدد **تم التحرير**.</span><span class="sxs-lookup"><span data-stu-id="292cc-527">Select **Done editing**.</span></span>
13. <span data-ttu-id="292cc-528">حدد  **حفظ إلى Lifecycle Services**، وقم بإقران تسجيل المهام الجديدة بمكتبة BPM نفسها وعملية الأعمال التي اقترن بها تسجيل المهام الأصلي.</span><span class="sxs-lookup"><span data-stu-id="292cc-528">Select **Save to Lifecycle Services**, and associate the new task recording with the same BPM library and business process that the original task recording was associated with.</span></span> <span data-ttu-id="292cc-529">للحصول على المزيد من المعلومات، راجع قسم [إنشاء تسجيل المهام وحفظها في قسم مكتبة BPM](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="292cc-529">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>
14. <span data-ttu-id="292cc-530">انتقل إلى مكتبة BPM، وحدد **مزامنة حالات الاختبار** لاستبدال تسجيل المهام المرفق بحالة الاختبار في Azure DevOps، كما هو موضح في قسم [اختبار المزامنة من BPM إلى Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="292cc-530">Go to the BPM library, and select **Sync testcases** to overwrite the task recording that is attached to the test case in Azure DevOps, as described in the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>
15. <span data-ttu-id="292cc-531">افتح RSAT، وحدد **تحميل** لإعادة تحميل كل حالات الاختبار الموجودة في مجموعة الاختبار.</span><span class="sxs-lookup"><span data-stu-id="292cc-531">Open RSAT, and select **Load** to reload all the test cases in the test suite.</span></span> <span data-ttu-id="292cc-532">ويجب إعادة إنشاء ملفات الأتمتة والمعلمات من أجل توفير حالة الاختبار المناسبة عن طريق تحديد حالة الاختبار ثم تحديد **جديد \> إنشاء ملفات اختبار التنفيذ والمعلمات**، كما هو موضح في قسم [حالات اختبار التحميل والتشغيل](#load-and-run-test-cases).</span><span class="sxs-lookup"><span data-stu-id="292cc-532">You must regenerate the automation and parameter files for the appropriate test case by selecting the test case and then selecting **New \> Generate Test Execution and Parameter files**, as described in the [Load and run test cases](#load-and-run-test-cases) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="292cc-533">إذا تم ترك ملف معلمة Excel مفتوحًا، فستفشل عملية التجديد.</span><span class="sxs-lookup"><span data-stu-id="292cc-533">If the Excel parameter file was left open, regeneration will fail.</span></span> <span data-ttu-id="292cc-534">لذا تأكد من إغلاق ملف معلمة Excel لحالة الاختبار قبل إنشاء ملف معلمة Excel الجديد.</span><span class="sxs-lookup"><span data-stu-id="292cc-534">Therefore, make sure that the Excel parameter file for the test case is closed before you generate the new Excel parameter file.</span></span>

16. <span data-ttu-id="292cc-535">حدد **تحرير** لفتح ملف معلمة Excel الجديد.</span><span class="sxs-lookup"><span data-stu-id="292cc-535">Select **Edit** to open the new Excel parameter file.</span></span> <span data-ttu-id="292cc-536">وستشاهد إدخالًا **متغيرِّا جديدًا محفوظًا** عبر الإنترنت في السطر 9.</span><span class="sxs-lookup"><span data-stu-id="292cc-536">You will see a new **Saved variable** entry on line 9.</span></span> <span data-ttu-id="292cc-537">يتم حفظ هذا المتغير، **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**، في ملف XML الخاص بتسجيل المهام ويمكن استخدامه في الاختبارات اللاحقة.</span><span class="sxs-lookup"><span data-stu-id="292cc-537">This variable, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, is saved in the task recording's XML file and can be used in subsequent tests.</span></span>

    ![إدخال المتغير المحفوظ](./media/setup_rsa_tool_94.png)

#### <a name="create-a-new-test-case"></a><span data-ttu-id="292cc-539">إنشاء حالة اختبار جديدة</span><span class="sxs-lookup"><span data-stu-id="292cc-539">Create a new test case</span></span>

1. <span data-ttu-id="292cc-540">انتقل إلى مكتبة **RSAT** BPM</span><span class="sxs-lookup"><span data-stu-id="292cc-540">Go to the **RSAT** BPM library.</span></span>
2. <span data-ttu-id="292cc-541">حدد عملية **نموذج عملية أعمال الدعم**، ثم، على اليسار، حدد **وضع التحرير**.</span><span class="sxs-lookup"><span data-stu-id="292cc-541">Select the **Sample Support Business Process** process, and then, on the right, select **Edit mode**.</span></span>
3. <span data-ttu-id="292cc-542">قم بتغيير قيمة حقل **الاسم** وحقل **الوصف** **لإصدار منتج**.</span><span class="sxs-lookup"><span data-stu-id="292cc-542">Change the value of both the **Name** field and the **Description** field to **Release a product**.</span></span> <span data-ttu-id="292cc-543">ثم حدد **حفظ**.</span><span class="sxs-lookup"><span data-stu-id="292cc-543">Then select **Save**.</span></span>

    ![تم تغيير الاسم والوصف لإصدار منتج](./media/setup_rsa_tool_95.png)

#### <a name="create-a-new-task-recording-that-has-a-validate-function"></a><span data-ttu-id="292cc-545">إنشاء تسجيل مهام جديدة لها وظيفة التحقق من الصحة</span><span class="sxs-lookup"><span data-stu-id="292cc-545">Create a new task recording that has a Validate function</span></span>

- <span data-ttu-id="292cc-546">قم بإنشاء تسجيل مهام لتحرير المنتج الذي تم إنشاؤه مسبقًا إلى الكيان القانوني USRT.</span><span class="sxs-lookup"><span data-stu-id="292cc-546">Create a task recording to release the product that was created earlier to the USRT legal entity.</span></span> <span data-ttu-id="292cc-547">للحصول على المزيد من المعلومات، راجع قسم [إنشاء تسجيل المهام وحفظها في قسم مكتبة BPM](#create-a-task-recording-and-save-it-to-the-bpm-library).</span><span class="sxs-lookup"><span data-stu-id="292cc-547">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="292cc-548">بالنسبة لحالات الاختبار المتسلسلة، نوصي دائمًا بالعثور على السجل الذي تحتاجه أو تصفيته عن طريق *كتابة قيمة الحقل يدويًا*.</span><span class="sxs-lookup"><span data-stu-id="292cc-548">For chained test cases, we always recommend that you find or filter for the record that you require by *manually typing the value of the field*.</span></span> <span data-ttu-id="292cc-549">وبهذه الطريقة، يمكن للأداة تحديد السجل الذي يجب اتخاذ الإجراء ضده في حالة الاختبار اللاحقة.</span><span class="sxs-lookup"><span data-stu-id="292cc-549">In that way, the tool can determine the record that the action must be taken against in the subsequent test case.</span></span>

    ![تسجيل مهام جديدة لها وظيفة التحقق من الصحة](./media/setup_rsa_tool_96.png)

    <span data-ttu-id="292cc-551">كما يوضح الرسم التوضيحي السابق، بعد العثور على المنتج باستخدام عامل التصفية السريع، ولكن قبل تحديد **إصدار المنتجات**، يجب أن تتحقق من قيمة حقل **رقم المنتج** للتأكد من أن معرف المنتج هو معرف المنتج الذي تم إنشاؤه مسبقًا.</span><span class="sxs-lookup"><span data-stu-id="292cc-551">As the preceding illustration shows, after the product is found by using the Quick Filter, but before you select **Release products**, you validate the value of the **Product number** field to make sure that the product ID is the product ID that was created earlier.</span></span> <span data-ttu-id="292cc-552">وللتحقق من صحة القيمة، انقر بزر الماوس الأيمن على حقل **رقم المنتج**، ثم حدد **مسجل المهام \> التحقق من الصحة \> القيمة الحالية**.</span><span class="sxs-lookup"><span data-stu-id="292cc-552">To validate the value, right-click the **Product number** field, and then select **Task recorder \> Validate \> Current Value**.</span></span>

    ![التحقق من صحة القيمة الحالية](./media/setup_rsa_tool_97.png)

#### <a name="save-the-task-recording-to-bpm"></a><span data-ttu-id="292cc-554">احفظ تسجيلات المهام إلى BPM</span><span class="sxs-lookup"><span data-stu-id="292cc-554">Save the task recording to BPM</span></span>

1. <span data-ttu-id="292cc-555">بعد اكتمال تسجيل المهام، حدد **حفظ إلى Lifecycle Services**.</span><span class="sxs-lookup"><span data-stu-id="292cc-555">After the task recording is completed, select **Save to Lifecycle Services**.</span></span>

    ![خيارات الحفظ](./media/setup_rsa_tool_98.png)

2. <span data-ttu-id="292cc-557">يتم تحميل معلومات المكتبة من LCS.</span><span class="sxs-lookup"><span data-stu-id="292cc-557">Library information is loaded from LCS.</span></span>

    ![مؤشر التقدم](./media/setup_rsa_tool_99.png)

3. <span data-ttu-id="292cc-559">حدد مكتبة BPM لإقران تسجيل المهام بها.</span><span class="sxs-lookup"><span data-stu-id="292cc-559">Select the BPM library to associate the task recording with.</span></span> <span data-ttu-id="292cc-560">بالنسبة لهذا البرنامج التعليمي، حدد مكتبة **RSAT** BPM التي تم إنشاؤها مسبقًا وعملية أعمال **إصدار منتج** ضمنها.</span><span class="sxs-lookup"><span data-stu-id="292cc-560">For this tutorial, select the **RSAT** BPM library that was created earlier and the **Release a product** business process under it.</span></span> <span data-ttu-id="292cc-561">ثم حدد **موافق**.</span><span class="sxs-lookup"><span data-stu-id="292cc-561">Then select **OK**.</span></span>

#### <a name="sync-bpm-to-azure-devops"></a><span data-ttu-id="292cc-562">مزامنة BPM إلى Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="292cc-562">Sync BPM to Azure DevOps</span></span>

1. <span data-ttu-id="292cc-563">انتقل إلى مكتبة BPM، وافتح مكتبة **RSAT**.</span><span class="sxs-lookup"><span data-stu-id="292cc-563">Go to the BPM library, and open the **RSAT** library.</span></span>
2. <span data-ttu-id="292cc-564">حدد **مزامنة VSTS** ثم **مزامنة حالات الاختبار**.</span><span class="sxs-lookup"><span data-stu-id="292cc-564">Select **VSTS sync** and then **Sync test cases**.</span></span> <span data-ttu-id="292cc-565">للحصول على المزيد من المعلومات، راجع قسم [اختبار المزامنة من BPM إلى Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops).</span><span class="sxs-lookup"><span data-stu-id="292cc-565">For more information, see the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>

    <span data-ttu-id="292cc-566">بعد اكتمال المزامنة سيظهر عنصر العمل الجديد وحالة الاختبار المقابلة لعملية الأعمال **إصدار منتج** في Azure DevOps في **لوحات المعلومات \> عناصر العمل**.</span><span class="sxs-lookup"><span data-stu-id="292cc-566">After the synchronization is completed, the new work item and the corresponding test case for the **Release a product** business process appear in Azure DevOps at **Boards \> Work Items**.</span></span>

#### <a name="add-the-new-test-case-to-the-existing-test-suite"></a><span data-ttu-id="292cc-567">إضافة حالة الاختبار الجديدة إلى مجموعة الاختبار الحالية</span><span class="sxs-lookup"><span data-stu-id="292cc-567">Add the new test case to the existing test suite</span></span>

1. <span data-ttu-id="292cc-568">انتقل إلى **خطط الاختبار \> خطط الاختبار**، وحدد خطة **خطة اختبار RSAT**.</span><span class="sxs-lookup"><span data-stu-id="292cc-568">Go to **Test plans \> Test plans**, and select the **RSAT Test Plan** plan.</span></span>
2. <span data-ttu-id="292cc-569">حدد **إضافة موجود**.</span><span class="sxs-lookup"><span data-stu-id="292cc-569">Select **Add existing**.</span></span>
3. <span data-ttu-id="292cc-570">في الصفحة **إضافة حالات اختبار إلى مجموعة**، حدد **تشغيل الاستعلام**.</span><span class="sxs-lookup"><span data-stu-id="292cc-570">On the **Add test cases to suite** page, select **Run query**.</span></span>
4. <span data-ttu-id="292cc-571">حدد حالة الاختبار الجديدة التي تم إنشاؤها **لإصدار منتج**، ثم حدد **إضافة حالات اختبار** في الركن الأيسر السفلي من الصفحة (لا يظهر هذا الزر في الرسم التوضيحي التالي).</span><span class="sxs-lookup"><span data-stu-id="292cc-571">Select the new test case that was created for **Release a product**, and then select **Add test cases** in the lower-right corner of the page (this button isn't shown in the following illustration).</span></span>

    ![صفحة "إضافة حالات اختبار إلى مجموعة"](./media/setup_rsa_tool_100.png)

    <span data-ttu-id="292cc-573">لدي مجموعة الاختبار الآن حالتي اختبار.</span><span class="sxs-lookup"><span data-stu-id="292cc-573">The test suite now has two test cases.</span></span>

    ![توجد حالتا اختبار الآن في مجموعة الاختبار.](./media/setup_rsa_tool_101.png)

#### <a name="load-test-cases-into-rsat"></a><span data-ttu-id="292cc-575">تحميل حالات الاختبار إلى RSAT</span><span class="sxs-lookup"><span data-stu-id="292cc-575">Load test cases into RSAT</span></span>

1. <span data-ttu-id="292cc-576">افتح RSAT، وحدد **تحميل**.</span><span class="sxs-lookup"><span data-stu-id="292cc-576">Open RSAT, and select **Load**.</span></span>
2. <span data-ttu-id="292cc-577">يتم تحميل حالات الاختبار، وستتلقى تحذيرًا ينص على ما يلي: "هذا الإجراء سيحل محل ملفات بيانات اختبار Excel، وستفقد التغييرات المحلية.</span><span class="sxs-lookup"><span data-stu-id="292cc-577">The test cases are loaded, and you receive a warning that states, "This action will overwrite Excel test data files, local changes will be lost.</span></span> <span data-ttu-id="292cc-578">فهل تريد المتابعة?"</span><span class="sxs-lookup"><span data-stu-id="292cc-578">Do you want to proceed?"</span></span> <span data-ttu-id="292cc-579">حدد **نعم** لتحديث ملفات معلمات Excel في النظام المحلي ولكن ليس ملفات معلمات Excel التي تم تحميلها إلى Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="292cc-579">Select **Yes** to update the Excel parameter files in the local system but not the Excel parameter files that were uploaded to Azure DevOps.</span></span>

    ![رسالة تحذير](./media/setup_rsa_tool_102.png)

    <span data-ttu-id="292cc-581">يتم تحميل حالتي الاختبار، إلى جانب ملف معلمة Excel لحالة الاختبار الأولى.</span><span class="sxs-lookup"><span data-stu-id="292cc-581">Both the test cases are loaded, together with the Excel parameter file for the first test case.</span></span> <span data-ttu-id="292cc-582">ونظرًا لأنك حددت **تحميل** في آخر عملية تشغيل، سيتم سحب ملفات المعلمات من Azure DevOps.</span><span class="sxs-lookup"><span data-stu-id="292cc-582">Because you selected **Upload** in the last run, the parameter files are pulled from Azure DevOps.</span></span>

    ![تم تحميل حالات الاختبار](./media/setup_rsa_tool_103.png)

3. <span data-ttu-id="292cc-584">حدد حالة الاختبار الثانية فقط، ثم حدد **جديد \> إنشاء ملفات تنفيذ الاختبار والمعلمة**.</span><span class="sxs-lookup"><span data-stu-id="292cc-584">Select only the second test case, and then select **New \> Generate test execution and parameter files**.</span></span>

#### <a name="edit-the-excel-parameter-file"></a><span data-ttu-id="292cc-585">تحرير ملف معلمة Excel</span><span class="sxs-lookup"><span data-stu-id="292cc-585">Edit the Excel parameter file</span></span>

1. <span data-ttu-id="292cc-586">حدد حالة الاختبار الثانية فقط، ثم حدد **تحرير** لفتح ملف معلمة Excel المقابل.</span><span class="sxs-lookup"><span data-stu-id="292cc-586">Select only the second test case, and then select **Edit** to open the corresponding Excel parameter file.</span></span>
2. <span data-ttu-id="292cc-587">انسخ المتغير المحفوظ **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** (راجع قسم [تعديل تسجيل مهام موجودة لإنشاء متغير محفوظ](#modify-an-existing-task-recording-to-create-a-saved-variable)) من حالة الاختبار الأولى إلى جميع الحقول التي يتم استخدام رقم المنتج فيها.</span><span class="sxs-lookup"><span data-stu-id="292cc-587">Copy the **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** saved variable (see the [Modify an existing task recording to create a saved variable](#modify-an-existing-task-recording-to-create-a-saved-variable) section) from the first test case into all the fields where the product number is used.</span></span> <span data-ttu-id="292cc-588">وفي هذه الحالة، يمكنك نسخ المتغير إلى حقل **رقم المنتج** وحقل **التحقق من صحة رقم المنتج** في الورقة **EcoResProductListPage**.</span><span class="sxs-lookup"><span data-stu-id="292cc-588">In this case, you copy the variable into the **Product number** and **Validate Product number** fields on the **EcoResProductListPage** sheet.</span></span>

    ![حقل رقم المنتج وحقل التحقق من صحة رقم المنتج](./media/setup_rsa_tool_104.png)

    > [!NOTE]
    > <span data-ttu-id="292cc-590">يمكن اجتياز المتغيرات بين الاختبارات فقط أثناء تشغيل الاختبار نفسه.</span><span class="sxs-lookup"><span data-stu-id="292cc-590">Variables can be passed between tests only during the same test run.</span></span> <span data-ttu-id="292cc-591">ويجب أن تطابق أسماء المتغيرات تطابقًا تامًا.</span><span class="sxs-lookup"><span data-stu-id="292cc-591">The names of the variables must match exactly.</span></span>

3. <span data-ttu-id="292cc-592">حدد **حفظ**، ثم قم بإغلاق مصنف Excel.</span><span class="sxs-lookup"><span data-stu-id="292cc-592">Select **Save**, and then close the Excel workbook.</span></span>
4. <span data-ttu-id="292cc-593">حدد **تحميل** لحفظ التغييرات التي أجريتها على ملف معلمة Excel.</span><span class="sxs-lookup"><span data-stu-id="292cc-593">Select **Upload** to save the changes that you made to the Excel parameter file.</span></span>

#### <a name="run-the-chained-test-cases"></a><span data-ttu-id="292cc-594">تشغيل حالات الاختبار المتسلسلة</span><span class="sxs-lookup"><span data-stu-id="292cc-594">Run the chained test cases</span></span>

1. <span data-ttu-id="292cc-595">حدد كلتا الحالتين، ثم حدد **تشغيل**.</span><span class="sxs-lookup"><span data-stu-id="292cc-595">Select both the test cases, and then select **Run**.</span></span>
2. <span data-ttu-id="292cc-596">تحقق من اجتياز كلتا حالتي الاختبار.</span><span class="sxs-lookup"><span data-stu-id="292cc-596">Verify that both test cases have passed.</span></span>
