...
قبل تضمين تطبيق لوحة من Power Apps في العميل، تحتاج إلى البحث عن تطبيق يحتوي على المرئيات أو الوظيفة المطلوبة أو إنشائه. لا يتضمن هذا الموضوع وصفًا مفصلاً لعملية إنشاء التطبيقات. إذا كان Power Apps جديدًا بالنسبة لك، فراجع [وثائق Power Apps](/powerapps/).
...
> [!NOTE]
> لا يمكنك استخدام هذه الآلية في الوقت الحال لتضمين التطبيقات المستندة إلى النماذج.  
...
3. تكوين التطبيق المضمن:
    ...
    - بالنسبة إلى **سياق الإدخال للتطبيق**، يمكنك اختياريًا تحديد الحقل الذي يحتوي على البيانات التي تريد تمريرها إلى التطبيق كإدخال. للحصول على تفاصيل حول كيفية وصول التطبيق إلى البيانات المرسلة من تطبيقات Finance and Operations، انظر القسم لاحقًا في هذا الموضوع بعنوان [إنشاء تطبيق يستفيد من البيانات المرسلة من تطبيقات Finance and Operations](#building-a-canvas-app-that-uses-data-that-is-sent-from-finance-and-operations-apps). 
        - بدءًا من الإصدار 10.0.19، سيتم أيضًا تمرير الكيان القانوني الحالي كسياق لتطبيق اللوحة عبر معلمة عنوان URL لـ **cmp**. لن يكون لهذا أي تأثير على تطبيق اللوحة المستهدف حتى يستخدم هذا التطبيق هذه المعلومات. 
...
1. [شارك تطبيق اللوحة](/powerapps/maker/canvas-apps/share-app) مع المستخدمين المناسبين، بحيث يمكنهم الوصول إلى التطبيق من Power Apps. 
