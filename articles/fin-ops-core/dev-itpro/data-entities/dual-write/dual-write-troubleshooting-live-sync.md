...
يوفر هذا الموضوع معلومات حول استكشاف أخطاء تكامل الكتابة الثنائية وإصلاحها بين تطبيقات Finance and Operations وMicrosoft Dataverse. على وجه التحديد، يوفر هذا الموضوع استكشاف الأخطاء وإصلاحها الذي يمكن أن يساعدك في إصلاح مشكلات المزامنة المباشرة.
...
قد تظهر رسالة الخطأ التالية عندما تحاول حفظ بيانات الجدول في تطبيق Finance and Operations:
...
لإصلاح المشكلة، تأكد من وجود بيانات مرجع المتطلبات الأساسية في كل من تطبيق Finance and Operations وDataverse. على سبيل المثال، إذا كان سجل العميل ينتمي إلى مجموعة عملاء محددة، فتأكد من وجود سجل مجموعة العملاء في Dataverse.

في حالة وجود بيانات في كلا المكانين، وقمت بالتأكد من أن المشكلة غير مرتبطة بالبيانات، فاتبع الخطوات التالية.

1. افتح كيان **DualWriteProjectConfigurationEntity** باستخدام المكون الإضافي لـ Excel. لاستخدام الوظيفة الإضافية، قم بتمكين وضع التصميم في وظيفة Excel الإضافية في Finance and Operations وأضف **DualWriteProjectConfigurationEntity** إلى ورقة العمل. لمزيد من المعلومات، راجع [عرض بيانات الكيان وتحديثها باستخدام Excel](../../office-integration/use-excel-add-in.md).
...
قد تستلم رسالة الخطأ "طلب غير صحيح" عند إنشاء البيانات في تطبيق Finance and Operations:
...
يحدث هذا الخطأ إذا تمت إعادة تعيين بيئة Dataverse بشكل غير صحيح عندما تحاول إنشاء البيانات في تطبيق Finance and Operations.

> [!IMPORTANT]
> إذا قمت بإعادة ربط البيئات، فيجب عليك إيقاف كافة تعيينات الكيانات قبل المتابعة في خطوات تقليل المخاطر.

لإصلاح المشكلة، يجب إكمال الخطوات في كل من تطبيق Dataverse وFinance and Operations.

1. في تطبيق Finance and Operations، اتبع الخطوات التالية:

    1. افتح كيان **DualWriteProjectConfigurationEntity** باستخدام المكون الإضافي لـ Excel. لاستخدام الوظيفة الإضافية، قم بتمكين وضع التصميم في وظيفة Excel الإضافية في Finance and Operations وأضف **DualWriteProjectConfigurationEntity** إلى ورقة العمل. لمزيد من المعلومات، راجع [عرض بيانات الكيان وتحديثها باستخدام Excel](../../office-integration/use-excel-add-in.md).
    2. حدد واحذف السجلات التي بها مشاكل في خريطة الكتابة المزدوجة والمشروع. سيكون هناك سجلان لكل تعيين للكتابة المزدوجة.
    3. قم بنشر التغييرات باستخدام وظيفة Excel الإضافية. هذه الخطوة هامة لأنها تحذف السجلات من الكيان والجداول الأساسية.
    4. للمساعدة في منع حدوث أخطاء عند إعادة ربط بيئات Finance and Operations أو Dataverse، تأكد من عدم بقاء أي تكوينات للكتابة المزدوجة.
...
3. في تطبيق Finance and Operations، اتبع الخطوات التالية:
...
## <a name="data-from-finance-and-operations-apps-isnt-synced-to-dataverse"></a>عدم مزامنة البيانات من تطبيقات Finance and Operations إلى Dataverse

أثناء المزامنة المباشرة، قد تواجه مشكله حيث تتم مزامنة جزء من البيانات فقط من تطبيقات Finance and Operations إلى Dataverse، أو عدم مزامنة البيانات على الإطلاق.
...
+ يجب تسجيل أحداث الأعمال لمصدر البيانات الذي تم تعيينه. قد تستخدم بعض مصادر البيانات صلة خارجية وقد يتم وضع علامة للقراءة عليها فقط في تطبيقات Finance and Operations. لا يتم تعقب مصادر البيانات هذه.
...
1. راجع تعيينات الحقول على صفحة إدارة الكتابة المزدوجة. إذا لم يتم تعيين حقل من تطبيقات Finance and Operations إلى Dataverse، فلن يتم تتبعه. على سبيل المثال، في التوضيح التالي، يتم تتبع حقل **الوصف** من Dataverse، ولكن ليس من تطبيقات Finance and Operations. لن يتم تعقب أي تغييرات تطرأ على هذا الحقل داخل تطبيقات Finance and Operations.
...
## <a name="error-when-you-create-a-record-where-multiple-records-are-sent-from-a-finance-and-operations-app-to-dataverse-in-the-same-batch"></a>حدوث خطأ عند إنشاء سجل حيث يتم إرسال سجلات متعددة من أحد تطبيقات Finance and Operations إلى Dataverse في نفس الدفعة

مع أي حركة، يقوم تطبيق Finance and Operations بإنشاء البيانات في دفعة وإرسالها كدفعة إلى Dataverse. إذا تم إنشاء سجلين كجزء من نفس الحركة، وقاما بالاشارة إلى بعضهما البعض، فقد تتلقي رسالة خطأ تشبه المثال التالي في تطبيق Finance and Operations:
...
لإصلاح المشكلة، قم بإنشاء علاقات الكيان في تطبيق Finance and Operations للإشارة إلى كيانين مرتبطين ببعضهما البعض، وأنه تجري معالجة السجلات المرتبطة في نفس الحركة.

## <a name="enable-verbose-logging-of-error-messages"></a>تمكين التسجيل المطول لرسائل الخطأ

في تطبيق Finance and Operations، قد تواجه أخطاء متعلقة ببيئة Dataverse. قد لا تحتوي رسالة الخطأ على النص الكامل للرسالة أو البيانات الأخرى ذات الصلة. للحصول على مزيد من المعلومات، يمكنك تمكين التسجيل المطول بتعيين العلامة **IsDebugMode** الموجودة في الكيان **DualWriteProjectConfigurationEntity** في كافة تكوينات المشروع في تطبيقات Finance and Operations.
...
قد تتلقي رسالة الخطأ التالية عند محاولة إضافة عنوان لعميل أو جهة اتصال في تطبيقات Finance and Operations أو Dataverse:
...
عند إنشاء عميل في Dataverse، يتم إنشاء رقم طرف جديد. يتم عرض رسالة الخطأ عند مزامنة سجل العميل، إلى جانب الطرف، مع تطبيقات Finance and Operations، ولكن يوجد بالفعل سجل عميل له رقم طرف مختلف.
...
في Dataverse، يوجد تسلسل رقمي في الجدول **msdyn_party**. عند إنشاء حساب في Dataverse، يتم إنشاء طرف جديد (على سبيل المثال **Party-001** من النوع **مؤسسة**). يتم إرسال هذه البيانات إلى تطبيق Finance and Operations. إذا تمت إعادة تعيين بيئة Dataverse، أو إذا كانت بيئة Finance and Operations مرتبطة ببيئة Dataverse مختلفة، ثم تم إنشاء سجل جهة اتصال جديد في Dataverse، فسيتم إنشاء قيمة طرف جديدة تبدا **Party-001**. وفي هذه المرة، سيكون سجل الطرف الذي تم إنشاؤه هو **Party-001** من النوع **شخص**. عند مزامنة هذه البيانات، تعرض تطبيقات Finance and Operations رسالة الخطأ السابقة ، نظرا لأن سجل **Party-001** من النوع **مؤسسة** موجود بالفعل.
