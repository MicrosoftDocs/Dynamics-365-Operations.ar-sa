...
description: توضح هذه المقالة وظيفة الطرف ودفتر العناوين العالمي للكتابة المزدوجة.
...
في تطبيقات التفاوض الخاصة بالعملاء ، عندما يقوم مستخدم بإدخال العناوين في **علامة التبويب ملخص** الخاصة بصفحه **الحساباتا** ، تتوافق حقول العنوان **والعنوان** مع **2** عناوين الفواتير **والتسليم** على **التوالي** . ومع ذلك ، عندما يقوم مستخدم بإنشاء عنوان بريدي في تطبيقات المعالجات المالية والعمليات ، سيظهر أول عنوانين من سجل العميل في الحقلين ، **Address1** و **Address2** ويكون لدي المستخدم الخيار لتغيير الغرض الخاص بالعنوان **للتسليم** و **الفاتورة**

![علامة التبويب ملخص للعناوين البريدية.](media/party-gab-image5.png)

بصورة ممائلة، تتوافق حقول **العنوان 1** ، و **العنوان 2** ، و **العنوان 3** في علامة التبويب **ملخص** في صفحة **جهة الاتصال** مع عناوين **الأعمال** ، و **التسليم** ، و **الفاتورة** ، على التوالي.
...
خلال عملية تأهيل العميل المتوقع، يمكنك توفير رقم هاتف عمل ورقم هاتف محمول. ويعتبر رقم هاتف العمل رقم الهاتف الأساسي إذا كان **IsMobile=No** ويعتبر رقم الهاتف المحمول رقم الهاتف الثانوي إذا كان **IsMobile=Yes**.
...
2. قم بتثبيت جميع حلول المتطلبات الأساسية، كما هو موضح في [حزمة تنسيق تطبيق الكتابة المزدوجة المنفصلة‬](separated-solutions.md).
...
7. يتم تحديث تعيينات الكيان التالية لوظيفة الطرف، لذا يجب تطبيق أحدث إصدار علي هذه التعيينات.
    ...
    ---|---|---
    `CDS Parties (msdyn_parties)`| 1.0.0.2 | هذه خريطة جديدة تمت إضافتها كجزء من هذا الإصدار.
    `Contacts V2 (msdyn_contactforparties)`| 1.0.0.6 | هذه خريطة جديدة تمت إضافتها كجزء من هذا الإصدار.
    ...
    `CDS postal address history V2 (msdyn_postaladdresses)` | 1.0.0.2 | هذه خريطة جديدة تمت إضافتها كجزء من هذا الإصدار.
    ...
    `CDS Address roles (msdyn_addressroles)` | 1.0.0.0 | هذه خريطة جديدة تمت إضافتها كجزء من هذا الإصدار.
...
12. إذا كنت مستخدمًا موجودًا للكتابة المزدوجة، فأكمل الخطوة 11، ثم يمكنك تشغيل الخرائط بالترتيب التالي. إذا كنت عميلاً جديدًا للكتابة المزدوجة، فيمكنك المتابعة على الفور. في حاله ظهور رسالة خطأ تنص على "فشل التحقق من صحة المشروع. حقل الوجهة مفقود... "، فافتح الخريطة، وحدد **تحديث الجداول**، ثم شغّل الخريطة.
    ...
    تطبيق التمويل والعمليات | تطبيق Customer Engagement  
    ----------------------------|------------------------
    ...
    [أدوار عناوين CDS](mapping-reference.md#301) | msdyn_addressroles

> [!NOTE]
> خريطة `CDS Contacts V2 (contacts)` هي الخريطة التي أوقفتها في الخطوة 1. عند محاولة تشغيل خرائط أخرى، قد تظهر هذه الخرائط من 2 في قائمه التوابع. لا تقم بتشغيل هذه الخرائط.
>
...
| [أدوار عناوين CDS](mapping-reference.md#301) |msdyn\_addressroles|

لمزيد من المعلومات، راجع [مرجع تعيين الكتابة المزدوجة](mapping-reference.md).

## <a name="address-roles-as-a-multi-select-drop-down-list"></a>أدوار العناوين كقائمة منسدلة متعددة الاختيارات
بإمكان عنوان بريدي أو عنوان إلكتروني أن يخدم أكثر من غرض واحد. على سبيل المثال، بإمكان العنوان البريدي أن يعمل كعنوان فوترة وعنوان تسليم. وفي هذه الحالات، بإمكان المستخدم أن يحدد **الفاتورة** و **التسليم** في القائمة المنسدلة، كما يظهر في الشكل التوضيحي التالي. 

![القائمة المنسدلة الغرض/الدور.](media/purpose.png)
