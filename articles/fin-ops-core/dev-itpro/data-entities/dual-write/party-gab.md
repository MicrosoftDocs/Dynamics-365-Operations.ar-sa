...
تتوفر العناوين الكترونيه علي هذه الشبكة فقط. في الإصدارات المستقبلية، ستتم أزاله كل حقول العناوين البريدية والعناوين الإلكترونية من علامات التبويب الأخرى (علي سبيل المثال، علامتي التبويب **ملخص** و **التفاصيل**).

## <a name="setup"></a>الإعداد

1. قم بتثبيت أحدث إصدار (2.2.2.60 أو إصدار أحدث) [لحل تنسيق تطبيق الكتابة المزدوجة](https://aka.ms/dual-write-app).

2. ثبت [حلول الكتابة المزدوجة للطرف ودفتر العناوين العمومي](https://aka.ms/dual-write-gab).

3. قم بإيقاف الخرائط التالية لأنها غير مطلوبه بعد الآن. بدلا من ذلك، قم بتشغيل خريطة `Contacts V2 (msdyn_contactforparties)`.

    + جهات اتصال CDS V2 وجهات الاتصال (تشير إلى جهات اتصال العملاء)
    + جهات اتصال CDS V2 وجهات الاتصال (تشير إلى جهات اتصال الموردين)

4. يتم تحديث تعيينات الكيان التالية لوظيفة الطرف، لذا يجب تطبيق أحدث إصدار علي هذه التعيينات.
...
5. قبل تشغيل الخرائط أعلاه، يجب تحديث مفاتيح التكامل يدويا كما هو موضح في الخطوات التالية. ثم حدد **حفظ**.
...
6. في Dataverse، تزيد حدود الأحرف الخاصة بقواعد الكشف عن التكرارات من 450 إلى 700 حرفا. يتيح لك هذا الحد أضافه مفتاح واحد أو أكثر إلى قواعد الكشف عن التكرارات. قم بتوسيع قاعده الكشف عن التكرارات لجدول **الحسابات** من خلال اعداد الحقول التالية.
...
7. قم بتوسيع قاعده الكشف عن التكرارات لجدول **جهات الاتصال** من خلال اعداد الحقول التالية.
...
8. إذا كنت مستخدما جديدا للكتابة المزدوجة، اتبع الإرشادات الموجودة في [الترقية إلى الطرف ونموذج دفتر العناوين العمومي](upgrade-party-gab.md) وقم بترقيه البيانات.

9. قم بتشغيل الخرائط بالترتيب التالي. في حاله ظهور خطا ينص علي فشل التحقق من صحة المشروع. حقل الوجهة مفقود... "، ثم افتح المخطط وحدد **تحديث الجداول**. ثم قم بتشغيل الخريطة.
...
+ لا تتدفق العناوين الإلكترونية من جدول `msdyn_partyelectronicaddress` إلى حقول العناوين الإلكترونية في جدولي **الحساب** و **جهة الاتصال**. ونحن نقوم بالتخطيط لحل هذه المشكلة في إصدار تزايدي. لن تتم الكتابة فوق البيانات الموجودة في حقول العنوان الكتروني الموجودة علي جدولي **الحساب** و **جهة الاتصال**.
