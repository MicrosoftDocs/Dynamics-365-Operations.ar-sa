...
ms.locfileid: "2769696"
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="f581b-103">الأسئلة المتداولة حول التمهيد باستخدام بيانات الشركة</span><span class="sxs-lookup"><span data-stu-id="f581b-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../includes/banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="f581b-104">لماذا أحتاج إلى التمهيد؟</span><span class="sxs-lookup"><span data-stu-id="f581b-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="f581b-105">قد يكون لديك مثيل Common Data Service موجود أو مثيل تطبيق Dynamics 365 آخر باستخدام بيانات العمل، وتريد تمكين اتصال الكتابة الثنائية مقابل هذا التطبيق.</span><span class="sxs-lookup"><span data-stu-id="f581b-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="f581b-106">في هذه الحالة، تحتاج إلى تمهيد Common Data Service أو بيانات تطبيق Dynamics 365 الأخرى باستخدام معلومات الشركة قبل تمكين اتصال الكتابة الثنائية.</span><span class="sxs-lookup"><span data-stu-id="f581b-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="f581b-107">متى ينبغي عليَّ استخدام التمهيد؟</span><span class="sxs-lookup"><span data-stu-id="f581b-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="f581b-108">يجب عليك استخدام التمهيد قبل تمكين خرائط كيان الكتابة الثنائية (أثناء الخطوة رقم 5).</span><span class="sxs-lookup"><span data-stu-id="f581b-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="f581b-109">لإعداد اتصال الكتابة الثنائية بين مثيلات Finance and Operations وCommon Data Service أو تطبيق Dynamics 365 آخر، قم بتسجيل الدخول إلى تطبيق Finance and Operations كمسؤول.</span><span class="sxs-lookup"><span data-stu-id="f581b-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="f581b-110">انتقل إلى وحدة **إدارة البيانات**، وانقر فوق الزر **الكتابة الثنائية**.</span><span class="sxs-lookup"><span data-stu-id="f581b-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="f581b-111">يؤدي هذا إلى تشغيل **مكامل البيانات**.</span><span class="sxs-lookup"><span data-stu-id="f581b-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="f581b-112">أنشئ اتصال الكتابة الثنائية لشركة واحدة أو أكثر.</span><span class="sxs-lookup"><span data-stu-id="f581b-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="f581b-113">![إنشاء اتصال الكتابة الثنائية](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="f581b-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="f581b-114">قم بتمكين خريطة كيان **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="f581b-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="f581b-115">ويقوم هذا بمزامنة الشركات من تطبيق Finance and Operations مع Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="f581b-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="f581b-116">![تمكين خريطة الكيان](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="f581b-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="f581b-117">قم بتشغيل كود ربط النموذج على Common Data Service أو تطبيق Dynamics 365 آخر.</span><span class="sxs-lookup"><span data-stu-id="f581b-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="f581b-118">عند الانتهاء من التمهيد والنظام جاهز للمزامنة المباشرة، قم بتمكين خرائط الكيانات.</span><span class="sxs-lookup"><span data-stu-id="f581b-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="f581b-119">يؤدي تمكين خرائط الكيانات إلى تشغيل مزامنة البيانات الأولية لخرائط الكيانات الممكّنة.</span><span class="sxs-lookup"><span data-stu-id="f581b-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="f581b-120">تتم مزامنة البيانات المقابلة للشركات التي تم اختيارها في اتصال الكتابة الثنائية بين تطبيق Finance and Operations وCommon Data Service.</span><span class="sxs-lookup"><span data-stu-id="f581b-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="f581b-121">كيف يمكنني استخدام نموذج الكود؟</span><span class="sxs-lookup"><span data-stu-id="f581b-121">How to I use the code sample?</span></span>
<span data-ttu-id="f581b-122">كود النموذج هو تطبيق C# الذي يمكنك تحميله في Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="f581b-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="f581b-123">يأخذ تبعيات حزمة NuGet في SDK لـ Common Data Service، حيث يمكنك تحديث من خلال أدوات Visual Studio القياسية.</span><span class="sxs-lookup"><span data-stu-id="f581b-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="f581b-124">بعد إلغاء ضغط الحل وفتحه في Visual Studio واستعادة حزم NuGet، ابحث عن **TODO** في الكود.</span><span class="sxs-lookup"><span data-stu-id="f581b-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="f581b-125">تتم الإشارة إلى كل قرار تحتاج إلى اتخاذه حول كيف تريد تمهيد معلومات الشركة بواسطة **TODO**، مع كود نموذج لتنفيذ المتعارف عليه.</span><span class="sxs-lookup"><span data-stu-id="f581b-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="f581b-126">يعرض كود النموذج واحدًا من العديد من الطرق التي قد تقوم بها بتصنيف سجلات الكيانات حسب الشركة.</span><span class="sxs-lookup"><span data-stu-id="f581b-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="f581b-127">ومن خلال تغيير المنطق في أقسام **TODO**، يمكنك إنشاء تصنيف مخصص.</span><span class="sxs-lookup"><span data-stu-id="f581b-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="f581b-128">ماذا ينبغي أن أتوقع؟</span><span class="sxs-lookup"><span data-stu-id="f581b-128">What should I expect?</span></span>
<span data-ttu-id="f581b-129">وبشكل افتراضي، يتيح نموذج التطبيق توفير قاموس تعيينات أكواد وحدة العمل إلى الشركة.</span><span class="sxs-lookup"><span data-stu-id="f581b-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="f581b-130">ويتم تعيين أي كيان تقوم بتمهيده باستخدام **OwningBusinessUnit** تلقائيًا لاستخدام الشركة المحددة.</span><span class="sxs-lookup"><span data-stu-id="f581b-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="f581b-131">وسيقوم أي كيان دون استخدام حقل **OwningBusinessUnit**، مثل منتج، بتعيين الشركة استنادًا إلى التعيين بقيمه خالية من وحدات العمل.</span><span class="sxs-lookup"><span data-stu-id="f581b-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="f581b-132">يتوقع تطبيق وحدة التحكم معلمة واحدة، إما **–محاكاة** أو **–تطبيق**.</span><span class="sxs-lookup"><span data-stu-id="f581b-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="f581b-133">إذا استخدمت معلمة سطر الأوامر **-محاكاة**، فلن يتم تحديث أي بيانات.</span><span class="sxs-lookup"><span data-stu-id="f581b-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="f581b-134">يتم إنشاء ملفات **simulation_<entityname>.csv** فقط في نفس دليل الأداه، واحده لكل وحده من الكيانات التي قد يتم تحديثها.</span><span class="sxs-lookup"><span data-stu-id="f581b-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="f581b-135">يمكنك بشكل متكرر مراجعة هذه الملفات اثناء العمل لضمان قيام الكود بتحديث قيم الشركة كما هو متوقع.</span><span class="sxs-lookup"><span data-stu-id="f581b-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="f581b-136">عند الانتهاء من التحديثات التي تمت محاكاتها، ثم استخدم معلمة **–تطبيق**.</span><span class="sxs-lookup"><span data-stu-id="f581b-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="f581b-137">يؤدي هذا إلى تحديث كافة السجلات التي لها قيمه شركه غير صحيحه، علي دفعات من 1000 سجل بالمرة الواحدة (افتراضيًا).</span><span class="sxs-lookup"><span data-stu-id="f581b-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="f581b-138">يُستخدم الكود مرة واحدة كما هو موضح، بمعني انه يمكنك أعاده تشغيله و سيتم تحديث الشركات المعينة بشكل غير صحيح فقط.</span><span class="sxs-lookup"><span data-stu-id="f581b-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="f581b-139">عند التشغيل باستخدام **–تطبيق**، يقوم الكود بإخراج ملفات CSV للتغييرات التي تم إجراؤها، وتُسمى **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="f581b-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
