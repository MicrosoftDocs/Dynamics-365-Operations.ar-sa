...
9. حدد **التحقق من الصحة** لفحص مكون تعيين النموذج القابل للتحرير علي صفحة **مصمم تعيين النموذج** والتحقق من إمكانية الاستعلام عن تعبير `FILTER(Vendor, Vendor.AccountNum="US-101")` في مصدر بيانات **المورد**.
    ...
    ![التحقق من إمكانية الاستعلام عن التعبير الذي لديه الدالة FILTER في صفحة مصمم تعيين النموذج.](./media/er-components-inspections-04.gif)
...
## <a name="executability-of-an-expression-with-orderby-function"></a><a id="i18"></a>إمكانية تنفيذ تعبير باستخدام الدالة

تُستخدم دالة التقارير الإلكترونية [ORDERBY](er-functions-list-orderby.md) المضمنة لفرز مصدر بيانات التقارير الإلكترونية من النوع **[قائمة السجلات](er-formula-supported-data-types-composite.md#record-list)** المحدد كوسيطة للدالة.

يمكن [تحديد](er-functions-list-orderby.md#syntax-2) وسيطات الدالة `ORDERBY` لفرز السجلات في  جداول التطبيق أو طرق العرض أو كيانات البيانات عن طريق وضع استدعاء قاعدة بيانات فردية للحصول على البيانات المفروزة كقائمة سجلات. يتم استخدام مصدر بيانات من النوع **قائمة السجلات** كوسيطة لهذه الدالة ويحدد مصدر التطبيق الخاص بالاستدعاء.

تتحقق التقارير الإلكترونية مما إذا كان من الممكن إنشاء استعلام قاعدة بيانات مباشر إلى مصدر بيانات يشار إليه في الدالة `ORDERBY`. في حاله تعذر إنشاء الاستعلام المباشر، يحدث خطا في التحقق من الصحة في مصمم تعيين نموذج التقارير الإلكترونية. توضح الرسالة التي تتلقاها ان تعبير التقارير الإلكترونية الذي يتضمن وظيفة `ORDERBY` لا يمكن تشغيله في وقت التشغيل.

توضح الخطوات التالية كيفيه حدوث هذه المشكلة.

1. ابدأ بتكوين مكون تعيين نموذج التقارير الإلكترونية.
2. أضف مصدر بيانات من نوع **Dynamics 365 for Operations \\ سجلات الجداول**.
3. قم بتسمية مصدر البيانات الجديد **المورد**. في حقل **الجدول**، حدد **VendTable** لتحديد أن مصدر البيانات هذا سيطلب الجدول **VendTable**.
4. أضف مصدر بيانات من نوع **الحقل المحسوب**.
5. قم بتسمية مصدر البيانات الجديد **OrderedVendors**، وقم بتكوينه بحيث يحتوي على التعبير `ORDERBY("Query", Vendor, Vendor.AccountNum)`.
 
    ![تكوين مصادر البيانات في صفحة مصمم تعيين النموذج.](./media/er-components-inspections-18-1.png)

6. حدد **التحقق من الصحة** لفحص مكون تعيين النموذج القابل للتحرير على صفحة **مصمم تعيين النموذج** وتحقق من إمكانية الاستعلام عن التعبير في مصدر بيانات البيانات **OrderedVendors**.
7. قم بتعديل مصدر بيانات **المورد** عن طريق أضافه حقل متداخل لـ **نوع الحقل المحسوب** للحصول علي رقم حساب المورد الذي تم اقتطاعه.
8. قم بتسميه الحقل المتداخل الجديد **$AccNumber**، وقم بتكوينه بحيث يحتوي علي التعبير `TRIM(Vendor.AccountNum)`.
9. حدد **التحقق من الصحة** لفحص مكون تعيين النموذج القابل للتحرير على صفحة **مصمم تعيين النموذج** وتحقق من إمكانية الاستعلام عن التعبير في مصدر بيانات **المورّد**.

    ![التحقق من إمكانية الاستعلام عن التعبير في صفحة مصدر بيانات المورّد تعيين النموذج.](./media/er-components-inspections-18-2.png)

10. لاحظ حدوث خطأ في التحقق من الصحة، لأن مصدر بيانات **المورّد** يحتوي علي حقل متداخل من النوع **حقل محسوب** الذي لا يسمح بترجمة تعبير مصدر البيانات **OrderedVendors‎** إلى عبارة قاعدة البيانات المباشرة. يحدث الخطأ نفسه في وقت التشغيل إذا تجاهلت خطأ التحقق من الصحة وحددت **تشغيل** لتشغيل تعيين النموذج هذا.
...
بدلاً من أضافه حقل متداخل من النوع **حقل محسوب** إلى مصدر بيانات **المورّد**، أضف الحقل المتداخل **$AccNumber** إلى مصدر بيانات **FilteredVendors**، وقم بتكوين الحقل بحيث يحتوي على التعبير `TRIM(FilteredVendor.AccountNum)`. وبهذه الطريقة، يمكن تشغيل التعبير `ORDERBY("Query", Vendor, Vendor.AccountNum)` على مستوى قاعدة البيانات، وبعد ذلك يمكن حساب الحقل المتداخل **$AccNumber**.

#### <a name="option-2"></a>خيار 2

غيّر تعبير مصدر بيانات **FilteredVendors** من `ORDERBY("Query", Vendor, Vendor.AccountNum)` إلى `ORDERBY("InMemory", Vendor, Vendor.AccountNum)`. لا نوصي بتغيير التعبير لجدول يحتوي على كمية كبيرة من البيانات (جدول المعاملات)، لأنه  سيتم إحضار كافة السجلات، وسيتم ترتيب السجلات المطلوبة في الذاكرة. لذلك، يمكن ان يتسبب هذا الأسلوب في ضعف الأداء.

## <a name="obsolete-application-artifact"></a><a id="i19"></a>أداة تطبيق قديمة

عندما تقوم بتصميم مكون تعيين طراز طراز قاعده البيانات (er) أو مكون التنسيق الخاص ب ER ، يمكنك تكوين تعبير ER لاستدعاء artifact التطبيق في الطراز الخاص ب ER ، مثل جدول قاعده بيانات وأسلوب من فئة ، الخ. في إصدار ال10.0.30 المالية والإصدارات الأحدث ، يمكنك فرض الاداه المنتج ER (ER) لتحذيرك ان التطبيق المشار اليه تم تمييزه في التعليمات البرمجية المصدر كقديم. يمكن ان يكون هذا التحذير مفيدا لأنه عاده ما تتم أزاله النتائج الملموسة المهملة في نهاية من التعليمات البرمجية للمصدر. يتم الاعلام حول حاله العنصر الملموس باستخدام العنصر الملموس القديم في مكون "نموذج المستخدم القابل للتحرير" قبل ازالته من التعليمات البرمجية المصدر ، مما يساعد علي منع حدوث أخطاء في استدعاء التطبيقات الملموسة غير الموجودة للتطبيق من أحد مكونات ER في وقت التشغيل.

قم بتمكين ميزه التحقق **من صحة العناصر القديمة لمصادر** بيانات التقارير الكترونيه في **مساحة عمل أداره** الميزات لتقييم السمة القديمة لنتائج التطبيق اثناء فحص أحد مكونات ER القابلة للتحرير. السمة القديمة التي يتم تقييمها حاليا للأنواع التالية من نتائج التطبيق:
...
> [!NOTE]
> ويحدث التحذير اثناء فحص مكونات ER القابلة للتحرير الخاصة بمصدر بيانات يشير إلى عنصر ملموس قديم فقط عند استخدام مصدر البيانات هذا في ربط واحد علي الأقل لمكون ER هذا.

> [!TIP]
> عند استخدام الفئة [SysObsoleteAttribute‎](../dev-ref/xpp-attribute-classes.md#sysobsoleteattribute) لاعلام المحول البرمجي لإصدار رسائل التحذير بدلا من الأخطاء ، يقدم تحذير الفحص المحدد في تحذير التعليمات البرمجية للمصدر في وقت التصميم في FastTab **التفاصيل** في **الصفحة "مصمم** تعيين الطراز" أو **"مصمم** التنسيق".

يبين الرسم التوضيحي التالي تحذير التحقق من الصحة الذي يحدث عند ربط الحقل القديم `DEL_Email` من `CompanyInfo` جدول التطبيق بحقل نموذج بيانات باستخدام مصدر البيانات الذي تم تكوينه `company`.

![راجع تحذيرات التحقق من الصحة في علامة FastTab التفاصيل في صفحة مصمم تعيين النموذج.](./media/er-components-inspections-19a.png)
...
قم بتعديل تعيين الطراز أو التنسيق الذي تم تكوينه عن طريق أزاله كافة عمليات الربط إلى مصدر البيانات التي تشير إلى artifact التطبيق القديم.
