...
يمكنك تكوين [وجهات](electronic-reporting-destinations.md) لكل مكون إخراج (مجلد أو ملف) لتكوين تنسيق [إعداد التقارير الإلكترونية (ER)](general-electronic-reporting.md)المستخدم[](general-electronic-reporting.md#Configuration) لإنشاء مستند صادر. يستطيع المستخدمون الذين يقومون بتشغيل أحد تنسيقات ER من هذا النوع، ومن لهم حقوق الوصول المناسبة، تغيير إعدادات الوجهة المكونة في وقت التشغيل.
...
إذا كانت تنسيقات التقارير الإلكترونية المتعددة قابلة للتطبيق كقوالب تقارير للمستند المحدد، يتم عرض كافة وجهات التقارير الإلكترونية لكافة قوالب التقارير الإلكترونية القابلة للتطبيق في مربع الحوار وتتوفر لإجراء تسوية يدوية في وقت التشغيل.

إذا لم تكن قوالب تقارير [‏‫خدمات تقارير SQL Server‬ (SSRS)](SSRS-report.md) قابلة للتطبيق على المستند المحدد، فإنه يتم إخفاء التحديد القياسي لأبعاد إدارة الطباعة بشكل ديناميكي مخفي.

واعتبارًا من Finance الإصدار **10.0.31**، فإنه يمكنك تغيير أبعاد التقارير الإلكترونية المعينة في وقت التشغيل لمستندات الأعمال التالية:
...
لتنشيط القدرة علي تغيير وجهات التقارير الإلكترونية في وقت التشغيل، قم بتمكين **السماح بتسوية وجهات التقارير الإلكترونية في وقت التشغيل**  في مساحة عمل [إدارة الميزات](../../fin-ops/get-started/feature-management/feature-management-overview.md#the-feature-management-workspace) .

> [!IMPORTANT]
> بالنسبة لتقارير **إخطار دفع العميل** و **إخطار دفع المورد**، تتوافر إمكانية تغيير وجهات التقارير الإلكترونية يدويًا فقط في حالة تمكين إصدار تقييم  **ForcePrintJobSettings**.

[![ضبط وجهات التقارير الإلكترونية في وقت التشغيل.](./media/ERdestinaiotnChangeUI.jpg)](./media/ERdestinaiotnChangeUI.jpg)

> [!NOTE]
> عند تعيين الخيار **استخدام وجهة إدارة الطباعة** إلى **نعم**، يستخدم النظام وجهات التقارير الإلكترونية الافتراضية التي يتم تكوينها لتقارير التقارير الإلكترونية المحددة. يتم تجاهل كافة التغييرات اليدوية التي يتم إجراؤها في مربع الحوار. قم بتعيين الخيار  **‏‫استخدام وجهة إدارة الطباعة** إلى **لا** لمعالجة المستندات إلى وجهات التقارير الإلكترونية التي يتم تعريفها في مربع الحوار مباشرة قبل تشغيل التقارير.

لا تفترض مستندات العمل التالية تحديد المستخدم الصريح لإجراء عند التشغيل:
...
يتم استخدام المنطق التالي لتحديد الإجراء الذي يتم استخدامه أثناء معالجة التقارير السابقة:

- في حالة تمكين إصدار تقييم **ForcePrintJobSettings**:

    - إذا تم تعيين الخيار **‏‫استخدام وجهة إدارة الطباعة‬** على **نعم**، فإنه يتم استخدام إجراء **الطباعة** .
    - إذا تم تعيين الخيار **‏‫استخدام وجهة إدارة الطباعة‬** على **لا**، فإنه يتم استخدام إجراء **عرض** .

- في حالة عدم تمكين إصدار تقييم **ForcePrintJobSettings**:

    - إذا تم تعيين الخيار **‏‫استخدام وجهة إدارة الطباعة‬** على **نعم**، فإنه يتم استخدام إجراء **الطباعة** لتقارير **‏‫إخطار دفع العميل‬**  و **‏‫إخطار دفع المورد‬**.
    - إذا تم تعيين الخيار **‏‫استخدام وجهة إدارة الطباعة** على **لا**، فإنه يتم دائمًا استخدام قالب تقرير SSRS الافتراضي للتقارير **إخطار دفع العميل‬** و **إخطار دفع المورد**، بغض النظر عن أية إعدادات تقارير إلكترونية يتم تكوينها.
    - يتم دائمًا استخدام إجراء **الطباعة** لتقارير **كشف حساب العميل** و **‏‫إشعار الفائدة‬** و **إشعار خطاب التحصيل**.

بالنسبة للمنطق السابق، يمكن استخدام إجراءات **الطباعة** أو **العرض** لتكوين وجهات التقارير الإلكترونية التي تعتمد على الإجراءات. في وقت التشغيل، يتم فقط تصفية وجهات التقارير الإلكترونية التي يتم تكوينها لإجراء محدد في مربع الحوار.
