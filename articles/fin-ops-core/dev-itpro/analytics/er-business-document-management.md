...
وبدلاً من ذلك، يمكنك استيراد تكوينات تنسيقات التقارير الإلكترونية التي تم نشرها رسميًا من Microsoft Dynamics Lifecycle Service (LCS). علي سبيل المثال، لإكمال هذا الاجراء، يمكنك استيراد أحدث إصدار من تكوين تنسيق التقارير الإلكترونية لـ **فاتورة النص الحر (Excel)**. سيتم استيراد نموذج بيانات التقارير الإلكترونية المقابل وتكوينات تعيينات نماذج التقارير الإلكترونية تلقائيا.
...
>[!NOTE]
> كما تحتاج إلى تمكين **تجربة واجهة المستخدم التي تشبه Office لإدارة مستندات العمل** لاستخدام واجهة إدارة مستندات العمل الجديدة
...
1. في مساحة عمل إدارة مستندات العمل، حدد **المستند الجديد**.

![صفحة مساحة عمل إدارة مستندات الأعمال](./media/BDM_overview_new_template1.png)

2. حدد المستند الذي تريد استخدامه كقالب.

![صفحة مساحة عمل إدارة مستندات الأعمال](./media/BDM_overview_new_template2.png)

3. انقر فوق **إنشاء مستند**
4. في حقل **العنوان**، قم بتغيير عنوان القالب القابل للتحرير إذا لزم الأمر. سيتم استخدام النص لتسمية تكوين تنسيق التقارير الإلكترونية لذي يتم إنشاؤه تلقائيًا. لاحظ أنه ستوضع علامة بشكل تلقائي على إصدار المسودة لهذا التكوين **(نسخة تقرير FTI للعميل (GER)**) الذي سيحتوي على القالب الذي تم تحريره من أجل تشغيل تنسيق التقارير الإلكترونية هذا للمستخدم الحالي. وفي الوقت نفسه، سيتم استخدام القالب الأصلي الذي لم يتم تعديله من تكوين تنسيق التقارير الإلكترونية الأساسي لتشغيل تنسيق التقارير الإلكترونية هذا لأي مستخدم آخر.
5. في الحقل‏‎ **الاسم**، قم بتغيير اسم المراجعة الأولى للقالب القابل للتحرير التي سيتم إنشاؤها تلقائيًا.
6. في الحقل‏‎ **التعليق**، قم بتغيير الملاحظة للمراجعة التي تم إنشاؤها تلقائيًا للقالب القابل للتحرير.
7. حدد **موافق** لتأكيد بدء عملية التحرير

![صفحة مساحة عمل إدارة مستندات الأعمال](./media/BDM_overview_new_template3.png)

يتوفر خيار **المستند الجديد‎** دائمًا لقالب في تكوين تنسيق التقارير الإلكترونية الذي يقدمه موفر آخر (Microsoft في هذا المثال). وعند النقر فوق **مستند جديد**، تشاهد جميع القوالب المملوكة للموفرين الحاليين والآخرين. بعد اختيار القالب، سيتم فتحه للتحرير. يتم بعد ذلك تخزين القالب الذي تم تحريره في تكوين تنسيق تقارير إلكترونية جديد يتم إنشاؤه بشكل تلقائي.
...
3. في الحقل‏‎ **التعليق**، قم بتغيير الملاحظة للمراجعة التي تم إنشاؤها تلقائيًا للقالب القابل للتحرير.
    ...
    ![صفحة مساحة عمل إدارة مستندات الأعمال](./media/BDM_overview_new_template4.png)
...
#### <a name="i-selected-edit-document-but-instead-of-opening-the-bdm-template-editor-page-in-finance-and-operations-i-have-been-sent-to-the-office-365-web-page"></a>لقد قمت بتحديد **تحرير مستند**، ولكن بدلاً من فتح صفحة **محرر قالب BDM** في Finance and Operations، تم إرسالي إلى صفحة ويب Office 365.
