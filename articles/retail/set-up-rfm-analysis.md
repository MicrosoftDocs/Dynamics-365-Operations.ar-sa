...
<span data-ttu-id="9c6d6-105">تحليل الحداثة والتكرار والقيم النقدية (RFM) هو أداة تسويق تستخدمها مؤسستك لتقييم البيانات التي يتم إنشاؤها بواسطة مشتريات العميل.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-105">Recency, frequency, and monetary (RFM) analysis is a marketing tool that your organization can use to evaluate the data that is generated by customer purchases.</span></span> <span data-ttu-id="9c6d6-106">بعد إعداد تحليل RFM، يتم تعيين مجموع RFM المحسوب للعملاء عند إجراء عمليات الشراء.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-106">After you set up RFM analysis, customers are assigned a calculated RFM score as they make purchases.</span></span> <span data-ttu-id="9c6d6-107">يمكن أن يكون مجموع RFM تصنيف من ثلاثة أرقام أو عدد تجميع، اعتمادًا على كيفية تكوين المؤسسة لتحليل RFM.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-107">The RFM score can be a three-digit rating or an aggregate number, depending on how your organization has configured RFM analysis.</span></span> <span data-ttu-id="9c6d6-108">إليك كيفية عمل تصنيف إذا كانت مؤسستك تستخدم تصنيفًا مكوناً من ثلاثة أرقام للنتيجة:</span><span class="sxs-lookup"><span data-stu-id="9c6d6-108">Here’s how the rating works if your organization uses a three-digit rating for the score:</span></span>

- <span data-ttu-id="9c6d6-109">الرقم الأول هو تصنيف حداثة العميل، ويقصد به الطريقة التي أجرى بها العميل مؤخرًا عملية شراء من مؤسستك.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-109">The first digit is the customer's recency rating, which is how recently the customer made a purchase from your organization.</span></span> 
- <span data-ttu-id="9c6d6-110">الرقم الثاني هو تصنيف تكرار العميل، ويقصد به عدد مرات قيام العميل بإجراء مشتريات من مؤسستك.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-110">The second digit is the customer's frequency rating, which is how often the customer makes purchases from your organization.</span></span> 
- <span data-ttu-id="9c6d6-111">والرقم الثالث هو تصنيف القيمة النقدية للعميل، ويقصد به المبلغ الذي ينفقه العميل عندما يقوم بإجراء مشتريات من مؤسستك.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-111">The third digit is the customer's monetary rating, which is how much the customer spends when he makes purchases from your organization.</span></span> 

<span data-ttu-id="9c6d6-112">على سبيل المثال، قامت مؤسستك بتعيين التصنيفات على مقياس من 1 إلى 5، حيث يمثل 5 أعلى تصنيف.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-112">For example, your organization has set the ratings on a scale of 1 through 5, where 5 is the highest rating.</span></span> <span data-ttu-id="9c6d6-113">وفي هذه حالة، يخبرك تصنيف العميل 535 بالمعلومات التالية فيما يتعلق بالعميل:</span><span class="sxs-lookup"><span data-stu-id="9c6d6-113">In this case, a customer rating of 535 tells you the following information about the customer:</span></span>

-   <span data-ttu-id="9c6d6-114">**تصنيف الحداثة 5** – أجرى العميل عملية شراء مؤخرًا.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-114">**Recency rating of 5** – The customer recently made a purchase.</span></span>
-   <span data-ttu-id="9c6d6-115">**تصنيف التكرار 3** – يشتري العميل منتجات من المؤسسة الخاصة بك بمعدل تكرار متوسط.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-115">**Frequency rating of 3** – The customer purchases products from your organization moderately often.</span></span>
-   <span data-ttu-id="9c6d6-116">**تصنيف القيمة النقدية 5** – عندما يقوم العميل بشراء، وينفق مبلغًا كبيرًا من المال.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-116">**Monetary rating of 5** – When the customer makes a purchase, he spends a significant amount of money.</span></span>

<span data-ttu-id="9c6d6-117">إذا كانت مؤسستك تستخدم رقمًا مجمعًا للمصدر، فإنه تتم إضافة التصنيفات الفردية معًا.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-117">If your organization uses an aggregate number for the score, the individual ratings are added together.</span></span> <span data-ttu-id="9c6d6-118">على سبيل المثال نفسه، العميل يمتلك تصنيف 13 (5 + 3 + 5).</span><span class="sxs-lookup"><span data-stu-id="9c6d6-118">For the same example, the customer has a rating of 13 (5 + 3 + 5).</span></span>

## <a name="to-set-up-rfm-analysis-for-the-customers-in-your-organization"></a><span data-ttu-id="9c6d6-119">لإعداد تحليل RFM للعملاء في مؤسستك.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-119">To set up RFM analysis for the customers in your organization.</span></span>

1.  <span data-ttu-id="9c6d6-120">انتقل إلى **مركز اتصال** > **دوري** > **تحليل RFM**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-120">Go to **Call center** > **Periodic** > **RFM analysis**.</span></span>

2.  <span data-ttu-id="9c6d6-121">في صفحة **تحليل RFM**، حدد **جديد**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-121">On **RFM analysis** page, select **New**.</span></span> <span data-ttu-id="9c6d6-122">في الحقل **تعريف RFM**، أدخل اسمًا لتعريف RFM.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-122">In the **RFM definition** field, enter a name for the RFM definition.</span></span> <span data-ttu-id="9c6d6-123">على سبيل المثال، يمكن استدعاء تعريف RFM-A.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-123">For example, you could call the definition RFM-A.</span></span>

3.  <span data-ttu-id="9c6d6-124">أدخل تاريخ البداية وتاريخ النهاية لتعريف RFM هذا.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-124">Enter a start date and end date for this RFM definition.</span></span>

4.  <span data-ttu-id="9c6d6-125">في علامة التبويب **عام**، قم بما يلي:</span><span class="sxs-lookup"><span data-stu-id="9c6d6-125">On the **General** FastTab, do the following:</span></span> 
...
1.  <span data-ttu-id="9c6d6-156">انتقل إلى **مركز اتصال** > **دفاتر اليومية** > **خدمة العملاء**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-156">Go to **Call center** > **Journals** > **Customer service**.</span></span> 

2.  <span data-ttu-id="9c6d6-157">في صفحة **خدمة العملاء**، ومن جزء **خدمة العملاء**، في حقول البحث، حدد نوع الكلمة الأساسية للبحث عن وإدخال نص البحث.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-157">On the **Customer service** page, in the **Customer service** pane, in the search fields, select the keyword type to search on and enter the search text.</span></span>

3.  <span data-ttu-id="9c6d6-158">حدد **بحث**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-158">Select **Search**.</span></span>

4.  <span data-ttu-id="9c6d6-159">على صفحة **البحث عن عميل**، حدد سجل العميل الذي تريده، ثم انقر فوق **حدد عميل**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-159">On the **Customer search** page, select the customer record that you want, and then click **Select customer**.</span></span> 

<span data-ttu-id="9c6d6-160">يتم عرض نقاط RFM في مجموعة **سجل الأمر** على الجانب الأيمن من صفحة **خدمة العملاء**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-160">The RFM score is displayed in the **Order history** group on the right side of the **Customer service** page.</span></span> 

## <a name="view-or-clear-the-history-of-an-rfm-analysis-record"></a><span data-ttu-id="9c6d6-161">عرض أو مسح محفوظات سجل تحليل RFM</span><span class="sxs-lookup"><span data-stu-id="9c6d6-161">View or clear the history of an RFM analysis record</span></span>
<span data-ttu-id="9c6d6-162">استخدم هذا الإجراء لعرض أو مسح محفوظات سجل تحليل RFM.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-162">Use this procedure to view or clear the history of an RFM analysis record.</span></span> 

1.  <span data-ttu-id="9c6d6-163">انتقل إلى **مركز اتصال** > **دوري** > **تحليل RFM**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-163">Go to **Call center** > **Periodic** > **RFM analysis**.</span></span>

2.  <span data-ttu-id="9c6d6-164">في صفحة **تحليل RFM**، حدد السجل الذي ترغب في عرضه.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-164">On the **RFM analysis** page, select the record that you want to view.</span></span>

3.  <span data-ttu-id="9c6d6-165">لعرض محفوظات السجل، حدد علامة التبويب السريعة **المحفوظات‬**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-165">To view the record history, select the **History** FastTab.</span></span>

4.  <span data-ttu-id="9c6d6-166">لمسح محفوظات السجل، حدد  **‏‫مسح المحفوظات‬**.</span><span class="sxs-lookup"><span data-stu-id="9c6d6-166">To clear the history of the record, select **Clear history**.</span></span>
