...
|----------|----------------------------------|-------------|-----------------------------|-----------------|----------------|---------------|--------------------|----------------------------|------------------------------|
| 100      | النقد في بداية الفترة (NP) |             |                             |                 |                |               |                    | حساب معدل = \[/BB\] | + Segment2 = \[1100\]         |
...
تضمين معادلة حساب في تعريف صف **+**، **-**، **\***، و **/**عوامل التشغيل، وكذلك **IF/ثم/ELSE** البيانات. بالإضافة إلى ذلك، يمكن أن يتضمن عملية حساب الخلايا الفردية والمبالغ المطلقة (الأرقام الفعلية المضمنة في المعادلة). ويمكن أن يصل طول المعادلة إلى 1,024 أحرف. ولا يمكن تطبيق العمليات الحسابية على الصفوف التي تحتوي على خلايا من نوع **ارتباط إلى الأبعاد المالية** (FD). وعلى الرغم من ذلك، يمكنك تضمين عمليات حسابية في صفوف متتالية ومنع طباعة تلك الصفوف، ثم تحديد مجموع صفوف العملية الحسابية.

### <a name="operators-in-a-calculation-formula"></a>العوامل في معادلة العملية الحسابية

تستخدم معادلة العملية الحسابية عوامل أكثر تعقيداً من معادلة إجمالي الصف. ومع ذلك، يمكنك استخدام **\***و **/**عوامل جنبا إلى جنب مع عوامل إضافية للضرب (\*) والقسمة (/) المبالغ. ولاستخدام نطاق أو مجموع في معادلة عملية حساب، يجب عليك استخدام علامة (@) أمام أي كود صف، إلا إذا كنت تستخدم عمود في تعريف صف. على سبيل المثال، لإضافة المبلغ 100 صف إلى المبلغ الموجود في الصف 330، يمكنك استخدام صيغة إجمالي الصف **أكثر من 100 330** أو معادلة الحساب **@100+@330**. **ملاحظة:** يجب عليك استخدام علامة (@) قبل كل كود صف تستخدمه في معادلة عملية حساب. وإلا، فإنه تتم قراءة الرقم كمبلغ مطلق. على سبيل المثال، الصيغة **@100+330** USD 330 بإضافة إلى المبلغ 100 صف. عند الإشارة إلى عمود في معادلة حساب، لا تكون العلامة (@) مطلوبة.
...
في هذا المثال، معادلة الحساب **@100+@330** يعني أن المبلغ الموجود في الصف 100 يضاف إلى المبلغ الموجود في الصف 330. الصيغة صف الإجمالي **340 + 370** تضيف في صف 340 إلى المبلغ الموجود في الصف 370. (في الصف 370 يبلغ المبلغ من معادلة الحساب.)

| كود الصف | الوصف                 | كود التنسيق | المعادلات/الصفوف/الوحدة ذات الصلة | التحكم في الطباعة | معدِّل الصف | الارتباط بالأبعاد المالية |
|----------|-----------------------------|-------------|----------------------------|---------------|--------------|------------------------------|
| 340      | النقد في بداية الفترة |             |                            | NP            | BB           | + الحساب =\[1100:1110\]       |
| 370      | النقد في بداية السنة   | CAL         | @100+@330                  | NP            |              |                              |
| 400      | النقد في بداية الفترة | TOT         | 340+370                    |               |              |                              |

عندما يكون الصف في تعريف صف يشتمل على كود تنسيق من **CAL**، وتقوم بإدخال عملية حسابية في خلية **المعادلات/الصفوف/الوحدات ذات الصلة**، يجب عليك أيضًا إدخال حرف العمود والصف المقترن في التقرير. على سبيل المثال، أدخل **A.120** لتمثيل العمود A، صف 120. بدلاً من ذلك، يمكنك استخدام علامة (@) للإشارة إلى كافة الأعمدة. على سبيل المثال، أدخل **@120**لتمثيل كافة الأعمدة في الصف 120. أي العمليات الحسابية التي لا تحتوي على حرف عمود أو علامة (@) من المفترض أن رقم الحقيقي. **ملاحظة:** إذا استخدمت رمز صف تسمية مرجع صف، يجب استخدام نقطة (.) كفاصل بين حرف العمود والتسمية (على سبيل المثال، **A.GROSS\_MARGIN/A.SALES**). إذا كنت تستخدم علامة (@)، فاصل غير مطلوب (على سبيل المثال، **@GROSS\_MARGIN/@SALES**).
...
-   لتقييد صيغة لأعمدة محددة، أدخل حرف العمود، علامة المساواة (**=**)، ومن ثم الصيغة.
...
يمكن إضافة عبارات **IF/THEN/ELSE** إلى أي عملية حساب صالحة البيانات واستخدامها مع تنسيق **CAL**. وتقوم بإدخال معادلات عمليات حساب **IF/THEN/ELSE**في الخلية في عمود **المعادلات/الصفوف/الوحدات ذات الصلة**. **IF/ثم/ELSE** حساب الصيغ استخدم التنسيق التالي: إذا &lt;عبارة صواب/خطأ&gt; ثم &lt;صيغة&gt; ELSE &lt;صيغة&gt;**ELSE &lt;صيغة&gt;** جزء من البيان اختيارية.
...
-   **إذا A.200&gt;0** (تقييم بسيط)
-   **إذا A.200&gt;0 و A.200&lt;10000** (بيان معقدة)
-   **إذا A.200&gt;10000 أو ((A.340/B.1200)\*2 &lt;1200)** (معقدة عبارة تحتوي على تعبيرات متعددة)
...
قد تكون معادلات **THEN** و**ELSE** أي عملية حساب صالحة من تعيينات القيم البسيطة للغاية للمعادلات المعقدة. على سبيل المثال، العبارة **A.200 إذا&gt;0 ثم A=B.200** يعني "إذا كانت القيمة في الخلية الموجودة في العمود A الصف 200 أكثر من 0 (صفر)، وضع القيمة من الخلية الموجودة في العمود B صف 200 في الخلية الموجودة في العمود A في الصف الحالي." تضع عبارة **IF/THEN** السابقة قيمة في عمود واحد في الصف الحالي. ومع ذلك، يمكنك أيضًا استخدام علامة (@) في تقييمات الصواب/الخطأ أو المعادلة لتمثيل كافة الأعمدة. فيما يلي بعض الأمثلة الأخرى الموضحة في الأقسام التالية:

-   **إذا A.200 &gt;0 ثم B.200**: إذا كانت القيمة في الخلية A.200 موجبة، يتم وضع القيمة من الخلية B.200 في كل عمود في الصف الحالي.
-   **إذا A.200 &gt;0 ثم @200**: إذا كانت القيمة في الخلية A.200 موجبة، يتم وضع القيمة من كل عمود في الصف 200 في العمود المناظر في الصف الحالي.
-   **إذا @200&gt;0 ثم @200**: إذا كانت القيمة في الصف 200 العمود الحالي إيجابية، يتم وضع القيمة من الصف 200 في نفس العمود في الصف الحالي.

### <a name="restricting-a-calculation-to-a-reporting-unit-in-a-row-definition"></a>تقييد عملية حساب على وحدة تقارير في تعريف صف

لتقييد حساب إلى وحدة واحدة تقارير في شجرة تقارير، حيث أن المبلغ الناتج غير الظاهرة لوحدة ذات مستوى أعلى، يمكنك استخدام **@Unit**التعليمات البرمجية في **الصيغ/صفوف/الوحدات المرتبطة** الخلية في تعريف الصف. **@Unit**يتم إدراج رمز في العمود B من شجرة التقارير، **"اسم وحدة"**. عند استخدام **@Unit**التعليمات البرمجية، القيم غير الظاهرة، ولكن الحساب يتم تقييم كل مستوى شجرة التقارير. **ملاحظة:** لاستخدام هذه الوظيفة، يجب أن تكون شجرة تقارير مرتبطة بتعريف الصف. ويمكن أن يشير صف عملية الحساب إلى صف عملية حساب أو صف بيانات مالية. ويتم تسجيل عملية الحساب في خلية **المعادلات/الصفوف/الوحدات ذات الصلة** في تعريف الصف وتقييد نوع البيانات المالية. يجب أن يستخدم الحساب حساب شرطي الذي يبدأ **إذا @Unit**البناء. إليك مثال: إذا @Unit(المبيعات) ثم @100ELSE 0 يتضمن حساب هذا المبلغ من الصف 100 في كل عمود من التقرير، ولكن فقط من أجل وحدة المبيعات. وإذا تمت تسمية عدة وحدات باسم المبيعات، يظهر المبلغ في كل وحدة من تلك الوحدات. بالإضافة إلى ذلك، يمكن أن يكون الصف 100 صف بيانات مالية ويمكن تحديده بأنها مخصص لعدم الطباعة. وفي هذه الحالة، يتم منع ظهور المبلغ في جميع الوحدات في الشجرة. ويمكنك أيضًا تحديد المبلغ لعمود واحد من التقرير، مثل العمود H، باستخدام قيد عمود لطباعة القيمة في عمود التقرير هذا فقط. ويمكنك تضمين مجموعات **OR** في عبارة **IF**. إليك مثال: إذا @Unit(المبيعات) أو @Unit(ساليسويست) ثم 5 ELSE @100يمكنك تحديد وحدة في تقييد نوع حساب بإحدى الطرق التالية:

-   أدخل اسم وحدة ليشمل الوحدات المطابقة. على سبيل المثال، **إذا @Unit(مبيعات)** يتيح حساب لأي وحدة يسمى "المبيعات"، حتى إذا كان هناك عدة وحدات المبيعات في شجرة التقارير.
-   أدخل الشركة واسم الوحدة لتقييد عملية الحساب على وحدات محددة في شركة معينة. على سبيل المثال، أدخل **إذا @Unit(القاهرة: المبيعات**) تقييد الحساب إلى وحدات المبيعات في الشركة القاهرة.
-   أدخل كود التسلسل الهرمي الكامل من شجرة التقارير لتقييد عملية الحساب على وحدة محددة. على سبيل المثال، أدخل **إذا @Unit(ملخص ^ القاهرة ^ الساحل الغربي ^ المبيعات)**. **ملاحظة:** للعثور على رمز التسلسل الهرمي الكامل، انقر بالزر الأيمن في تعريف شجرة التقارير، ثم حدد **نسخ معرف وحدة التقارير (كود H)**.

#### <a name="restrict-a-calculation-to-a-reporting-unit"></a>تقييد عملية حساب على وحدة تقارير

1.  في "مصمم التقرير"، انقر فوق **تعريفات الصفوف**، ثم قم بفتح تعريف صف لتعديله.
2.  انقر نقراً مزدوجاً فوق خلية **كود التنسيق**، ثم حدد **CAL**.
3.  انقر فوق **الصيغ/صفوف/الوحدات المرتبطة** الخلية، ثم قم بإدخال حساب شرطي الذي يبدأ **إذا @Unit**البناء.

### <a name="ifthenelse-statements-in-a-column-definition"></a>عبارات IF/THEN/ELSE في تعريف عمود

تعمل عبارات **IF/THEN/ELSE** على تمكين أي عملية حساب من الاعتماد على النتائج من أي عمود آخر. ويمكنك الإشارة إلى أعمدة أخرى، ولكن لا يمكنك الإشارة إلى خلية تقرير في عبارة **IF**. ويجب تطبيق أي عملية حساب على العمود بالكامل. على سبيل المثال، العبارة **"ب إذا"&gt;"ب ج آخر" ثم 100\*1.25** يعني، "إذا كان المبلغ الموجود في العمود B أكثر من 100، وضع القيمة من العمود B في **الحساب** عمود. إذا لم يكن المبلغ الموجود في العمود B أكبر من 100، فاضرب القيمة الموجودة في العمود C في 1.25 ووضع النتيجة في عمود **CALC**". واتبع دائماً عبارة **IF** بعبارة منطقية يمكن تقييمها كصحيحة أو خاطئة. ويمكن أن تحتوي المعادلات التي تستخدمها لكلٍّ من عبارة **THEN** وعبارة **ELSE** على مراجع إلى أي عدد من الأعمدة، ويمكن أن تكون هذه المعادلات معقدة كيفما تريد أن تجعلها. **ملاحظة:** لا يمكنك وضع نتائج عملية حسابية في أي عمود آخر. يجب أن تكون النتائج موجودة في العمود الذي يحتوي على المعادلة.
