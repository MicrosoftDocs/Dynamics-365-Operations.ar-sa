...
1.  في صفحة **مجموعات نماذج الصنف**، قم بإعداد مجموعة نماذج الصنف المشتملة على متوسط تكلفة النقل المحددة في حقل **نموذج المخزون**.
    | **ملاحظة **                                                                                                                                |
    |-----------------------------------------------------------------------------------------------------------------------------------------|
    | بشكل افتراضي، عندما يتم تحديد متوسط تكلفة النقل، يتم تحديد حقلي **ترحيل المخزون الفعلي** و**ترحيل المخزون المالي** أيضًا. |

2.  في صفحة **الترحيل**، قم بتعيين الحسابات إلى حسابات **فرق السعر لمتوسط تكلفة النقل** و**إعادة تقييم التكاليف لمتوسط تكلفة النقل** في علامة التبويب **المخزون**. يمكنك استخدام حساب **فرق سعر متوسط تكلفة النقل** عندما يلزم إنفاق التكلفة بشكل متناسب. ويحدث هذا بسبب فرق في التكلفة بين إيصال الشراء وفاتورة الشراء، وبسبب فرق بين كمية المخزون الأصلي والكمية الفعلية الحالية. استخدم حساب **إعادة تقييم التكلفة لمتوسط تكلفة النقل**، إذا كنت ترغب في تعديل متوسطة تكلفة النقل لمنتج بسعر وحدة جديد.
3.  في صفحة **المنتجات التي تم إصدارها**، قم بتعيين مجموعة نماذج الصنف لمتوسط تكلفة النقل لمنتج.
...
يمكن تحويل المنتجات إلى طريقة تقييم متوسط مخزون النقل. وعادةً ما يتم هذا النوع من التحويل في نهاية السنة، بعد إغلاق الشهر الأخير من السنة الحالية. ويتم ذلك باستخدام نموذج التكلفة الحالية للمنتج. ويمكنك تغيير طريقة حساب تكاليف المخزون من طريقة حساب تكاليف تستند إلى متوسط التكلفة أو التكلفة المعيارية لطريقة تستند إلى متوسط تكلفة النقل. إذا كنت تغير طريقة حساب التكاليف من طريقة حساب التكاليف القياسية إلى طريقة متوسط تكلفة النقل، يجب عليك إكمال المهام التالية:
...
| **ملاحظة **                                                                      |
|-------------------------------------------------------------------------------|
| التحويل من التكلفة القياسية إلى المتوسط المرجح للنقل عبارة عن عملية يدوية. |
...
من خلال متوسط النقل، تتحدد تكلفة المنتج بواسطة إيصال الشراء. وعند ترحيل فاتورة الشراء، إذا لم يكن هناك فرق في التكلفة بين إيصال استلام الشراء وفاتورة الشراء، فإنه يتم ضبط الفرق نسبيًا على المنتجات الحالية في المخزون، ويتم إنفاق أي مبلغ متبقي. وفي هذا المثال، يتم إنشاء أمر شراء وتلقيه بتكلفة واحدة، ويتم ترحيل فاتورة الشراء بتكلفة مختلفة.
...
يتم ترحيل الفرق في سعر الوحدة، 2.00، إلى فرق في السعر لحساب متوسط النقل عند ترحيل فاتورة الشراء. والسبب أن المنتجين قد تم شراؤهما بتكلفة مقدارها 20.00. وتم بيع أحد المنتجين بسعر وحدة بمبلغ 10.00. ترحيل فاتورة شراء بسعر وحدة من 12.00 بكمية من 2. لا يمكن ترحيل سعر الوحدة للمنتج في 14.00.

## <a name="moving-average-product-and-inventory-adjustment"></a>متوسط النقل للمنتج وتسوية المخزون
إذا كنت بحاجة إلى ضبط متوسط تكلفة النقل لمنتج، يُسمح بتعديلات المخزون اعتبارًا من تاريخ اليوم. ولا يمكنك تعيين تسوية مخزون إلى تاريخ سابق لتصحيح متوسط تكلفة نقل منتج. ولا يمكنك الحصول على تدفق التكلفة من خلال الحركات اللاحقة. وفي هذا المثال، تتم تسوية متوسط تكلفة النقل لمنتج.
1.  حدد المنتج الذي تريد تسوية متوسط تكلفة النقل له.
    | **Note**                                                                                    |
    |---------------------------------------------------------------------------------------------|
    | * * إعادة تقييم المتوسط المتحرك * * صفحة فحص المخزون المتاحة لمنتج. |

    يحتوي المنتج المحدد على كمية مرّحلة قدرها 1 مرحل، وقيمة مرّحلة قدرها 12.00، وتكلفة وحدة مرّحلة 12.00، وتكلفة وحدة بمبلغ 12.00.
2.  قم بتحديث حقل **تكلفة الوحدة** إلى 16.00. يحسب النظام الحقول المتبقية.
3.  يتم ترحيل التسوية.

| **ملاحظة **                                                        |
|-----------------------------------------------------------------|
| يمكنك فقط تسوية متوسط تكلفة النقل اعتبارًا من تاريخ اليوم. |
...
يدعم متوسط تكلفة النقل الأصناف التي تم إنتاجها. إذا كنت تخطط لاستخدام معدل النقل في بيئة إنتاج، **استخدام سعر التكلفة التقديرية** في التمرير * * محددات التحكم في الإنتاج * * يجب أن تكون الصفحة المحدد. ويعني هذا أنه يتم استخدام سعر التكلفة التي يتم حسابها أثناء تقدير بدلاً من سعر التكلفة الفعلي لقائمة مكونات الصنف.
