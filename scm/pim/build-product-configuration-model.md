...
يمكن أن تكون القيود الجدول المعرفة من قبل المستخدم أو معرف من قبل النظام.  
...
يتم تعريف قيد جدول المعرفة من قبل النظام بتحديد أي Microsoft Dynamics 365 لجدول عمليات لاستخدامه كمرجع ثم تحديد الحقول الموجودة في هذا الجدول لمن الأعمدة في القيد. صفوف الجدول قيد هي صفوف 365 Dynamics لجدول عمليات موجودة في وقت التكوين.  
...
اختبار نموذج مشابه لتشغيل جلسة عمل تكوين الفعلي. يمكن أن تقود إلى صفحات تكوين المستخدم والتحقق من بنية نموذج يدعم عملية التكوين. ويمكن للمستخدم التحقق من أن قيم السمات صحيحة، ومن أن أوصاف السمات ترشد المستخدم إلى تحديد القيم الصحيحة. وأخيراً، عند الانتهاء من جلسة اختبار، يحاول النظام إنشاء قائمة مكونات الصنف والمسار الذي يتوافق مع قيم السمة المحددة، ويعرض رسالة خطأ في حالة حدوث أي خطأ.
...
تم تنفيذ واجهة برمجة تطبيقات (API) مخصصة، بحيث يمكن للشركاء وغيرهم ممن لديهم ترخيص مطور توسيع قدرات نموذج تكوين المنتج. يتمثل الهدف الرئيسي لإنشاء إليه الشركاء والعملاء الذين يستخدمون "منشئ المنتج" الموجود ترحيل التعليمات البرمجية المضمنة في النماذج "منشئ المنتج" ل API. وبهذه الطريقة، يمكنهم ترحيل نماذجهم من "منشئ المنتج" إلى تكوين منتج. ومع ذلك، يمكن للشركاء والعملاء الاستفادة من استخدام واجهة برمجة التطبيقات لتوسيع نماذج تكوين المنتج الجديد.

### <a name="pcadaptor-class"></a>فئة محوّل الكمبيوتر

يتم تطبيق واجهة برمجة التطبيقات باستخدام مجموعة من فئات **محوّل الكمبيوتر** التي تعرض بنية بيانات نماذج تكوين المنتج. مثيل **بكادابتور** يجب إنشاء فئة لكل نموذج سيتم توسيع. عند الانتهاء من جلسة عمل تكوين، النظام التحقق مثيل من هذه الفئة ويعمل في حالة العثور عليه.  

يوضح الرسم التخطيطي التالي للتدفق العملية.  

[![رسم تخطيطي لسير](./media/product_configuration_2.png)](./media/product_configuration_2.png)  

رسم تخطيطي لسير API لتكوين المنتج
