...
&lt;اسم عنصر التحكم&gt;(&lt;اسم المفتاح&gt;) =&lt;مفتاح التعليمات البرمجية&gt;؛

فيما يلي شرح لأجزاء البنية:

-   **&lt;اسم عنصر التحكم&gt;** – اسم عنصر التحكم (على سبيل المثال، زر) التي يتم تقديمها في HTML.
-   **(&lt;اسم المفتاح&gt;) ** -اسم مفتاح لوحة المفاتيح التي تقوم بإنشاء الاختصار.
-   **&lt;مفتاح التعليمات البرمجية&gt;** -رمز رقمي لمفتاح الذي سيتم استخدامه كمفتاح اختصار.
...
Request.UserHostAddress=&lt;عنوان المضيف المستخدم&gt;| اسم المضيف =&lt;اسم المضيف المستخدم&gt;| Request.UserAgent=&lt;عامل المستخدم&gt;

فيما يلي شرح لأجزاء التعبير:

-   **&lt;عنوان المضيف المستخدم&gt;** –.NET أ التعبير العادي يطابق عنوان IP الطالب.
-   **&lt;اسم المضيف المستخدم&gt;** –.NET أ التعبير العادي يطابق اسم الشبكة للطالب.
-   **&lt;عميل المستخدم&gt;** –.NET أ التعبير العادي يطابق المعرف للمستعرض الذي يستخدم الطالب.

يتيح المثال التالي إمكانية استخدام Internet Explorer 8:

Request.UserHostAddress=. \*| اسم المضيف =. \*| Request.UserAgent=MSIE\\s8\\.0
...
يمكنك توسيع قائمة تنسيقات التاريخ المقبول لكل تثبيت. قد تكون هذه القدرة مفيدة، على سبيل المثال، إذا كنت تريد توفير تنسيق يتيح إدخال العامل للتواريخ على جهاز محمول بسهولة. ويتم تحديد التنسيق الافتراضي باللغة الافتراضية للمستخدم المحددة في حقل **اللغة** في صفحة **خيارات المستخدم**. (نفس الصفحة يستخدم أيضا لربط موظف بمستودع معين عمل مستخدم.) **ملاحظة:** "المدخل أجهزة الجوال المستودع" لا يستخدم الإعداد **التاريخ الوقت وتنسيق رقم** الحقل على **تفضيلات اللغة والمنطقة** الصفحة. لتغيير تنسيق تاريخ، يجب أن تكون على دراية بالتعبيرات العادية في Microsoft .NET Framework. لمزيد من المعلومات، راجع [التعبيرات العادية لـ .Net Framework](http://go.microsoft.com/fwlink/?LinkId=391260). لتحديد تنسيقات التاريخ، تحرير الملف Dates.ini الذي يقع في المحتوى\\الإعدادات\\Dates.ini على خادم "المدخل أجهزة الجوال المستودع". يستخدم هذا الملف التعبيرات العادية في NET. لتحديد تنسيق التاريخ. يجب أن تحتوي التعبير العادي على تعبيرات فرعية والتي تتيح إنشاء مجموعات lمسماة لليوم وللشهر وللسنة (ي ي ش ش س س)، كما هو موضح في المثال التالي:

^(? &lt;day&gt;\\d{2})(?&lt; month&gt;\\d{2})(?&lt; سنة&gt;\\{2} د) $
